...
2. [디렉토리 구조](#디렉토리-구조)
3. [빠른 시작](#빠른-시작)
4. [GUI 실행 (tablet_sim_gui.py)](#gui-실행-tablet_sim_guipy)
5. [밀도 기반 접촉 경도 모델](#밀도-기반-접촉-경도-모델)
6. [CLI 레퍼런스](#cli-레퍼런스)
7. [변경 이력](#변경-이력)
8. [의존성 설치](#의존성-설치)
9. [주의 사항](#주의-사항)

> **✅ 2026-06-01 업데이트 (최신)** — **GUI 프론트엔드** 추가 (`tablet_sim_gui.py`): 알약 무게·형상 입력 → 밀도·solref 실시간 계산, 경도 게이지 시각화, 단일/배치 시뮬 실행 원클릭 지원.  
> **✅ 2026-05-29 업데이트** — **밀도 기반 접촉 경도 모델** 추가 (`--density` / `--mass`): Hertzian Contact Theory를 기반으로 알약 밀도 → solref 시정수 τ 자동 계산.  
> → [GUI 실행 바로가기](#gui-실행-tablet_sim_guipy) | [밀도 기반 접촉 경도 상세 설명](#밀도-기반-접촉-경도-모델)
