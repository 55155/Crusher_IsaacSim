...
3. [디렉토리 구조](#디렉토리-구조)
4. [빠른 시작](#빠른-시작)
5. [GUI 실행 (tablet_sim_gui.py)](#gui-실행-tablet_sim_guipy)
6. [밀도 기반 접촉 경도 모델](#밀도-기반-접촉-경도-모델)
7. [CLI 레퍼런스](#cli-레퍼런스)
8. [변경 이력](#변경-이력)
9. [의존성 설치](#의존성-설치)
10. [주의 사항](#주의-사항)

> **✅ 2026-06-05 업데이트 (최신)** — **실제 모터 스펙(BL4281 + 1:212) 반영** 및 **준정적 조건** 구현: 환산 관성 주입으로 수치 안정성 확보, 실측 슬라이더 힘(600–650 N) 재현.  
> **✅ 2026-06-01 업데이트** — **GUI 프론트엔드** 추가 (`tablet_sim_gui.py`): 알약 무게·형상 입력 → 밀도·solref 실시간 계산, 경도 게이지 시각화, 단일/배치 시뮬 실행 원클릭 지원.  
> **✅ 2026-05-29 업데이트** — **밀도 기반 접촉 경도 모델** 추가 (`--density` / `--mass`): Hertzian Contact Theory를 기반으로 알약 밀도 → solref 시정수 τ 자동 계산.  
> → [하드웨어 사양 바로가기](#하드웨어-사양) | [GUI 실행 바로가기](#gui-실행-tablet_sim_guipy) | [밀도 기반 접촉 경도 상세 설명](#밀도-기반-접촉-경도-모델)
...
| 정격 토크 | 0.185 N·m |
| 무부하 속도 | 5,800 RPM |
| 로터 관성 | 72 g·cm²  (= 7.2 × 10⁻⁶ kg·m²) |
| 감속기 감속비 | **1 : 212** |
| 크랭크 출력 stall 토크 | ≈ **12.5 N·m**  (효율 ~32%) |
| 크랭크 무부하 속도 | ≈ **27.4 RPM**  (= 2.87 rad/s) |
| 크랭크 환산 관성 J_refl | **0.324 kg·m²**  (= J_motor × 212²) |

> 고감속비 감속기는 역구동 차단(self-locking) 특성이 있어 슬라이더에 부하가 걸려도 크랭크 위치가 유지됩니다.
...
θ = 90°  →  F = 12.5 / 0.02       = 625 N   ← 실측값과 일치 ✓
θ = 60°  →  F = 12.5 / 0.01732    = 722 N
θ = 30°  →  F = 12.5 / 0.01       = 1250 N  ← TDP 근처 증폭
```

---

### 시뮬레이션 모터 파라미터 (`crusher_velocity_ctrl.py`)

| 파라미터 | 값 | 의미 |
|----------|----|------|
| `MOTOR_STALL_TORQ` | `0.185` N·m | BL4281 모터 정격 토크 |
| `MOTOR_NOLOAD_RPM` | `5800` RPM | 무부하 속도 |
| `MOTOR_INERTIA_KG` | `72e-7` kg·m² | 로터 관성 |
| `GEAR_RATIO` | `212` | 감속비 |
| `CRANK_R_M` | `0.020` m | 크랭크 반경 |
| `ROD_L_M` | `0.080` m | 커넥팅 로드 길이 |
| `_TAU_STALL_CRANK` | `12.5` N·m | 크랭크 출력 stall 토크 (실측 역산) |
| `_J_REFL` | `0.3236` kg·m² | 크랭크 body 환산 관성 (J × n²) |
| `MOTOR_FORCELIM` | `12.5` N·m | velocity actuator forcerange 상한 |
| `VEL_KV_DEFAULT` | `≈ 14.9` N·m·s/rad | actuator 게인 (= τ_stall / ω_target) |
...
환산관성 미적용:  14.9 × 0.002 / 4.2e-5 ≈ 710  >> 2  →  수치 발산 ✗
환산관성 적용:    14.9 × 0.002 / 0.324  ≈ 0.09 << 2  →  안정 ✓
```

환산 관성은 `_build_model()` 내에서 `L4_Shaft_1` body의 `diaginertia`에 `_J_REFL`을 더하는 방식으로 주입됩니다.
...
## GUI 실행 (tablet_sim_gui.py)

> 커맨드라인 없이 마우스로 모든 파라미터를 입력하고 시뮬레이션을 실행할 수 있는 tkinter 기반 GUI입니다.
...
```
┌────────────────────────────────────────────────────────────────┐
│  ⚗ Crusher Tablet Simulator  —  밀도 기반 접촉 경도 자동 계산  │
├──────────────────────────────┬─────────────────────────────────┤
│  📋 알약 물성 입력            │  📊 계산 결과 (실시간)           │
│                              │                                 │
│  ○ 무게(mg) 입력             │  추정 부피   [ 320.5 ] mm³      │
│  ● 밀도(kg/m³) 직접 입력     │  추정 밀도   [  1200 ] kg/m³    │
│                              │  solref τ   [ 0.0080] s        │
│  무게    [ 500  ] mg         │  solimp d   [ 0.991 ]          │
│  밀도    [1200  ] kg/m³      │                                 │
│                              │  ─────────────────────────      │
│  반지름 R [ 8.0 ] mm         │  경도: ████████░░  Hard 65%    │
│  종횡비 AR[ 0.80]            │                                 │
│  왕관비 CV[ 0.25]            │  밀도→τ 참조표                  │
│                              │   900 → 0.0200  연질            │
│  STL 파일 [___path___] [📂]  │  1200 → 0.0080  기본값 ◀현재   │
│                              │  1800 → 0.0020  경질            │
├──────────────────────────────┴─────────────────────────────────┤
│  🚀 단일 시뮬레이션 │  📦 배치 시뮬레이션                       │
│  [☐ 그래프 저장]  [  ▶ 시뮬레이션 실행  ]                      │
├────────────────────────────────────────────────────────────────┤
│  📟 출력                                                        │
│  $ python crusher_tablet_sim.py tablet.stl --density 1200      │
│  ◆ Phase 1: 500 스텝 안정화...                                  │
│  ✔ Phase 1 완료                                                 │
└────────────────────────────────────────────────────────────────┘
```
...
| 기능 | 설명 |
|------|------|
| **입력 모드 토글** | 무게(mg) 입력 ↔ 밀도(kg/m³) 직접 입력 선택 |
| **실시간 계산** | 숫자 입력 즉시 → 부피·밀도·τ·solimp 자동 갱신 |
| **경도 게이지** | Soft(녹) → Medium(노) → Hard(주) → Very Hard(빨) 색상 표시 |
| **참조 테이블** | 현재 밀도에 가장 가까운 행 자동 하이라이트 |
| **콘솔 출력** | 서브프로세스 stdout 실시간 표시 (오류=빨강, 완료=녹색) |
| **단일 탭** | STL + 밀도 → `crusher_tablet_sim.py` 실행 |
| **배치 탭** | STL 폴더 전체 + 병렬/순차 선택 → `batch_tablet_sim.py` 실행 |

### 입력 모드별 동작

**무게 입력 모드** (`무게(mg) 입력` 선택)
- 무게(mg) + 형상 파라미터(R, AR, CV) → 부피 추정 → 밀도 자동 계산
- 측정된 실측 질량이 있을 때 사용
- 파일명에 `R`, `AR`, `CV` 파라미터가 포함된 STL과 연동

**밀도 직접 입력 모드** (`밀도(kg/m³) 직접 입력` 선택)
- 밀도(kg/m³)를 직접 입력 → τ 즉시 계산
- 배치 시뮬레이션에서 동일 밀도를 전체 적용할 때 사용
- 참고: 연질 ≈ 900, 일반 ≈ 1200, 경질 ≈ 1800 (kg/m³)
...
### 2026-06-05 — 실제 모터 스펙 반영 및 준정적 조건 구현

#### 핵심 변경

| 항목 | 변경 전 | 변경 후 |
|------|---------|---------|
| **MOTOR_FORCELIM** | 120 N·m (임의값) | **12.5 N·m** (실측 역산: F=625N × r=0.02m) |
| **VEL_KV_DEFAULT** | 60 → 3 (진동 수정) | **14.9** (= τ_stall / ω_target, 물리 근거 수치) |
| **환산 관성 주입** | 없음 | `_build_model()`에서 `L4_Shaft_1`에 J×n²=**0.324 kg·m²** 추가 |
| **모터 상수 추가** | 없음 | `GEAR_RATIO`, `MOTOR_STALL_TORQ`, `MOTOR_INERTIA_KG`, `CRANK_R_M`, `ROD_L_M` |

#### 배경: 진동 원인 및 해결 과정

kv=60이었을 때 stall 토크 = 60 × 0.838 = **50 N·m** (현실 모터의 ~20배).  
접촉 순간 속도 강하 → kv × Δω 보정 토크 과다 → 속도 오버슈트 → 진동.

| 단계 | kv | MOTOR_FORCELIM | 안정 지수 kv·dt/J | 결과 |
|------|----|----------------|-------------------|------|
| 초기 | 60 | 120 N·m | 710 (J≈크랭크만) | 진동 발생 |
| 1차 수정 | 3 | 3 N·m | 0.09 | 진동 해소, 힘 과소 |
| **최종** | **14.9** | **12.5 N·m** | **0.09** (환산관성 포함) | 안정 + 실측 재현 ✓ |

#### 슬라이더 힘 검증

실측 600–650 N ↔ 시뮬레이션 예측:
- τ_stall = 12.5 N·m, r = 0.02 m, θ = 90° → F = **625 N** ✓

#### 수정 파일

| 파일 | 수정 내용 |
|------|-----------|
| `MuJoCo_PlayGround/20260603/crusher_velocity_ctrl.py` | BL4281 스펙 상수 추가, 환산 관성 주입(`_build_model`), kv·forcelim 실측 재산정 |
| `README.md` | 하드웨어 사양 섹션 신규 추가 |
