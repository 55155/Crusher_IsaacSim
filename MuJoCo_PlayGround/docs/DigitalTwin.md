...
| 시험 | 결과 | 용도 |
|---|---|---|
| 단축 압축 F–δ | 힘–변위 초기 기울기 | → E 보정 표적 |
| Brazilian(직경압축) | F_break = 80 N | → σ_t |
| 위 30개 반복 | 파괴하중 분포 | → Weibull m ≈ 8 |
| Crusher 반복 타격 | F_peak별 파쇄 타수 | → F_peak–N_f 표적 |

볼록면이라 σ_t 는 **Pitt 식**:
`σ_t = 10F/(πD²)·[2.84·t/D − 0.126·t/W + 3.15·W/D + 0.01]⁻¹ = 800/(π·64)/1.82 ≈ 2.2 MPa`

**STEP 2 — E 보정 (1순위 항)**: 강제변위 FEM 반력 F(δ)를 실측 기울기에 맞춰 E 조정.
가정 E=2 GPa → 반력이 실측의 1/3 → **E ≈ 6 GPa** 로 수렴(MCC 현실값), ν=0.25 문헌
고정. 이제 σ_I 절대값[MPa]이 물리적 의미를 가진다.
...
`D += f(σ_I, σ_t),  D ≥ 1 → 파쇄, 그때 타수 = N_f`. **손상규칙 계수**를 실측
F_peak–N_f 곡선에 맞춰 조정(예: 60 N에서 예측 5 → 실측 3에 수렴). Weibull m 은
파쇄 *확률* 산포로 반영. → 이는 §7-4 Regime II(누적 손상) 케이스.
...
| 역할 | 항목 |
|---|---|
| 입력(측정) | σ_t=2.2 MPa, Weibull m≈8, ρ·porosity |
| 조정 항 | **E≈6 GPa**(F–δ로), **손상규칙 계수**(F_peak–N_f로), 접촉/봉투/전류 |
| 표적(맞춤) | F(δ), Brazilian σ_t, F_peak–N_f, 균열위치 |
| 출력(예측) | 신규 조건 N_f · 필요토크 · 균열패턴 |

> 한 줄: **E·손상계수는 "돌려 맞추는 항", σ_t는 "재서 넣는 입력", N_f는 "예측·검증
> 표적"** — 세 역할이 한 알약을 관통한다. 관련: [`fem_stress_field.py`](../../Crusher_Genesis/FEM/fem_stress_field.py), DataInventory.md §E.
