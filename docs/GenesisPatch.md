...
## 부록 — 조 자가충돌과 디스크립션 리밋 (2026-09-03)

패치와 직접 관계는 없으나 같은 씬에서 나온 것이라 여기 남긴다.

흡착 조를 0 까지 닫으면 두 컵 면이 서로 파고들어 접촉 배리어와 PD 가 싸운다.
`_prepare_suction_mjcf()` 로 MJCF 사본을 만들어 range 를 `-0.05 ~ -0.003` 으로
제한하면 사라진다(원본 에셋은 안 건드린다).

| | 명령 -0.42mm 시점 실제 | 스테이지 흔들림 |
|---|---|---|
| 리밋 없음 | -10.26 / -10.30mm | +1.89mm |
| 리밋 -3mm | -3.00 / -3.06mm | +0.01mm |

구동 방식·게인·미믹을 하나도 안 바꾸고 리밋만 걸어 가른 결과다. 자세한 것은
`docs/DigitalTwin.md` §23-5.
