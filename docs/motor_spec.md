...
> **각주 — 12.5 N·m 와 §4-1 의 10 N·m 은 왜 다른가.**
> 둘 다 실측 역산값이지만 **모집단이 다르다.**
> - **12.5 N·m** = θ=90° 벤치 1점(600–650 N)에서 `τ = F·r` 로 역산 → 시뮬의
>   `forcerange`·`kv` 상수. **상한(stall limit)** 성격.
> - **9~10 N·m** = force gauge robust-mean 7점(θ=30–120°, [`Real2Sim.md`](Real2Sim.md) §2)
>   전체를 `τ = F·r·sinθ` 로 역산한 값 (θ=30–90° 기준 τ = 9.0 ± 0.5 N·m,
>   로드 경사 보정 시 10.1 ± 0.5 N·m). §4-1 의 설계 토크 10 N·m 과 일치.
>
> 즉 **12.5 는 다각도 실측 대비 약 25 % 높게 잡혀 있다.** 시뮬에서는 이 차이를
> `Crusher_8env.py` 의 `FORCE_SCALE = 0.9453` 스칼라 캘리브레이션이 일부 흡수한다
> ([`DigitalTwin.md`](DigitalTwin.md) §7-7). 모터 선정 근거로 인용할 때는 **10 N·m**
> 를, 시뮬 상수를 설명할 때는 **12.5 N·m** 를 쓸 것.
...
### 4-1. 모터 사이징 — 요구 반력에서 거꾸로

위 §3·§4 는 "모터가 있을 때 얼마가 나오나"(정방향)이고, 설계 시에는 반대로
**요구 반력 → 크랭크 토크 → 모터 토크** 순으로 푼다. 8 RPM 저속이라 관성항은
무시할 수 있다(`m·r·ω²/F ≈ 10⁻⁵ ≪ 1`).

![모터 사이징 수식](motor_sizing_eqs.png)

```
τ_crank = F_req × r × sin θ_c          (준정적, 가상일)
τ_motor = τ_crank / (n × η)            (감속·손실)
F(θ)    = τ_motor × n × η / (r sin θ)  (검증)
```

| 단계 | 계산 | 결과 |
|------|------|------|
| 요구 반력 @ 접촉각 | F_req = 800 N, θ_c = 30° | — |
| 크랭크 토크 | 800 × 0.020 × sin 30° | **8.0 N·m** |
| 설계 토크 (안전계수 1.25) | 8.0 × 1.25 | **10 N·m** |
| 모터 토크 | 10 / (212 × 0.5) | **0.094 N·m** |
| 선정 | BL4281 정격 0.185 N·m | **여유 ×2** |

> `r·sin θ` 는 모멘트 팔이 아니라 **속도비 dx/dθ** [m/rad] 다. 같은 토크라도 각도에
> 따라 힘이 몇 배씩 달라지므로, **각도를 빼고 "몇 N 낼 수 있다"고 말할 수 없다.**
> τ=10 N·m 기준 가용 반력은 θ=30°에서 1000 N, θ=90°에서 500 N(최소).
> 800 N 이상은 θ ≤ 39°, 즉 **행정 끝 5.4 mm 구간**에서만 나온다 — 모터를 키우는 대신
> 충돌판을 사점 쪽에 붙여 기구 이득을 쓰는 것이 이 기구의 설계 요점.
