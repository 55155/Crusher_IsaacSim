...
### 20-6. 현재 권고 설정 [§21 로 대체됨]

> **이 절과 §20-4 는 §21 이 뒤집었다.** `WALL_KINEMATIC` 은 임시방편이 아니라
> 이 봉투에 대한 유일한 방법이고(봉투 반력 < 2N, §21-4), 아래 환경변수들은 전부
> 기본값으로 승격돼 더 이상 줄 필요가 없다(§21-6). 힘제어 복귀는 **목표가 아니다.**

```
(구) LAYOUT_FROM_STEP=1 WALL_KINEMATIC=1 CLAMP_TARGET_MM=-12.5 LEFTWALL_FLANGE_CONTACT=1
(신) 환경변수 없이 그냥 돌린다. 회수장치까지 보려면 RECOVER=1 CRUSH_SECONDS=15
```

## 21. 크랭크가 안 돈 이유 — `set` 과 `control` 을 섞으면 안 된다 (2026-08-31)

분쇄 구간에서 크랭크가 **8 RPM 지령에 0.02 RPM** 밖에 안 나왔다. 15초에 2.1도다.
원인을 찾는 데 하루가 걸렸고, **가설 일곱 개를 세워 여섯 개가 실측으로 기각됐다.**
그 과정 자체가 이 절의 본론이다 — 결론만 보면 같은 함정을 또 판다.

### 21-1. 결론부터

`full_workflow.py` 의 분쇄 루프가 벽 DOF 를 `set_dofs_position` 으로 잡고 있는데,
**같은 엔티티의 크랭크는 `control_dofs_velocity` 로 구동된다.** 이렇게 섞으면
`control` 쪽 DOF 가 죽는다. 분쇄 구간만 벽을 `control_dofs_position` 으로
되돌리자 **8.00 RPM** 에 정확히 도달했다.

이것은 이 세션이 만든 회귀다. 원래 코드는 `control` 이었는데 "clamp 와 일관성"을
이유로 `set` 분기를 넣은 것이 발단이었다.

### 21-2. 격리 매트릭스 — 이것이 증거다

`Crusher_only_20260831_spin.py`. 크러셔 단독 씬에서 변인을 하나씩 켠다.
게인·토크·타임스텝은 전부 고정(kp 2000 / kv 5000 / ±12.5 N·m / dt 5e-3).

| 환경 | IPC | 봉투 | SET | NOWELD | RPM |
|---|---|---|---|---|---|
| base | ✗ | ✗ | ✗ | ✗ | 6.902 |
| ipc | ✓ | ✗ | ✗ | ✗ | 6.945 |
| ipc_bag | ✓ | ✓ | ✗ | ✗ | 6.945 |
...
- **IPC 도 봉투도 영향이 0이다.** `ipc` 와 `ipc_bag` 이 소수점까지 같다.
- **`set` 단독으로 재현된다.** 봉투 없이 `set` 만으로 0.024 가 나오고, 이는
  `full_workflow` 의 0.02 와 일치한다.
- **폐루프도 필요조건이 아니다.** weld 를 실제로 떼도(`noweld_set`) 여전히 멈춘다.

### 21-3. 기각된 가설 여섯 개

전부 **한 변인만 바꾸지 않고 관측에 맞는 이야기를 만든** 결과였다.

| # | 가설 | 기각 근거 |
|---|---|---|
| 1 | 크랭크-슬라이더 **사점**(−180° 파킹) | τ 40배에 ω 40배로 완전 선형. 사점이면 breakaway 가 보여야 한다 |
| 2 | **링크 관성**이 크다 | MJCF 실측 전체 가동 링크 1.297 kg, 구동계 0.866 kg. r=0.02m 반사관성 3.5e−4 kg·m² |
| 3 | **`kv=5000`** 이 유효 관성을 부풀린다 | 실측 4점이 `ω=τ/kv` 에 정확히 맞았으나, 그 피팅이 요구하는 M=0.168 은 물리값의 **480배**. 런 B 가 kv=5000 으로 정상 회전 |
| 4 | **`substeps`/타임스텝** | 런 B 가 같은 `substep_dt=5e-3` 으로 회전. 누적 논증도 동일 — 증분 0.00248 rad/s 면 340 스텝에 ω_c 를 넘어야 한다 |
| 5 | **weld `solref` 시정수** 50배 완화 | 사실이고 로그에도 찍힌다(`timeconst is changed from 0.0002 to 0.01`). 그러나 런 B 가 그 상태로 회전한다 — 스핀업이 0.1s → 2s 로 느려질 뿐이다 |
| 6 | **IPC / 봉투(FEM)** | 위 매트릭스. `coup_links` 필터도, `IPC_CONSTRAINT_STRENGTH` 100→1 도, `coup_type` 변경도 전부 0.02 로 동일 |

가설 3이 가장 위험했다. 실측 3점이 식 위에 정확히 얹혀서 확신했는데, **물리
질량으로 검산하는 한 줄**이면 480배 어긋난 것을 바로 봤을 것이다. 수식이 데이터에
맞는 것과 그 수식이 옳은 것은 다르다.

### 21-4. 무엇이 확정됐고 무엇이 아닌가

**확정** — 같은 엔티티에서 `set` 과 `control` 을 섞으면 `control` 쪽이 죽는다.
반례처럼 보이는 로봇 팔은 **전 자유도가 `set`** 이라 죽을 `control` DOF 가 없다.

```
크러셔    벽 = set   +  크랭크 = control   ->  크랭크 죽음
로봇 팔   전부 set                          ->  정상
```

**미확정** — 정확한 메커니즘. `set_dofs_position` 은 맨 앞에서
`collider.reset()` + `constraint_solver.reset()` 을 부르고(`rigid_solver.py:2710`),
끝에서 `_is_forward_vel_updated = True` 를 세운다. 후자가 엔티티 단위 플래그라
**속도 적분이 건너뛰어질** 수 있고, 크랭크가 정확히 한 스텝치 증분
(`Δω = τ·dt/(M+kv·dt)`)에 머무는 것과 맞아떨어진다. 폐루프 가설이 기각된 이상
이쪽이 더 유력하지만 **검증하지 않았다.**

### 21-5. 같이 나온 것들

**(a) `set_dofs_position` 은 속도를 지우지 않는다.** 압착 중 위치가 −12.79mm 로
고정인데 `v=−58.8mm/s` 가 계속 찍혔다. 위치를 덮어써서 운동으로 안 나타날 뿐
속도는 쌓이고, 지령이 끊기는 순간 벽이 튄다(0.69mm 점프 후 −17.01mm 까지 밀림).
IPC 커플러도 같은 이유로 `set_dofs_velocity` 를 짝지어 부른다
(`coupler.py:1002`, "avoid double time integration"). → 기구학 구동 경로 전부에
`set_dofs_velocity(None)` 을 짝지었다.

**(b) 봉투 반력은 2N 미만이다 [§20-4 정정].** `control_dofs_force` 를 벽에 직접
건 것은 이번이 처음이다 — §20 이 "힘제어"라 부른 것은 전부
`control_dofs_position` + `force_range` 클램프였다. 스윕(`sweep_force_20260831.py`,
힘 5 × armature 2) 결과 **2N 으로도 벽이 하드스톱까지 닫히고 40N 과 정지 위치가
0.16mm 차이**다. 힘 균형점이 존재하지 않으므로 §11-5 의 "정지 위치 = 실효 압착
두께"라는 측정 논리는 **이 봉투로는 원리적으로 불가능**하고, `WALL_KINEMATIC` 은
임시방편이 아니라 유일한 방법이다. armature 는 발산만 막는다(10 은 5건 중 3건
발산, 100 은 5/5 완주). 실효 압착 두께는 힘이 아니라 **하드스톱이 정한다**
(본체면 잔여 0.6~1.1mm) — 재현성은 오히려 좋다.

**(c) 실링/측벽 정렬 버그.** `LAYOUT_FROM_STEP` 이 실링축을 Y→X 로 옮기면서
`BAG_DY_FROM_FINGER` 가 0 이 되어 봉투중심 보정이 **조용히 멈췄다.** 파지 오프셋
28mm 가 무보정으로 남아 봉투가 프레임 중심에서 28mm 밀린 채 계속 돌고 있었다
(좌 실링은 프레임 밖, 우 실링은 개구 한가운데). `BAG_DX_FROM_FINGER` 를 추가해
봉투 중심을 프레임 중심에 맞췄다 — 실링 밴드(중심 23~32mm)가 좌우 측벽
(27.5~32.5mm, `hull_001`/`hull_003`)에 4.5mm 씩 물린다.

**(d) 계측 버그.** 회수장치 IK 도달오차가 yaw 를 360° 훑어도 19.7~20.1mm 로
평탄했다. IK 는 **TCP**(링크원점 + `FINGER_TCP_LOCAL`)를 푸는데 검사는 링크
원점을 목표와 비교하고 있어서, 완벽히 풀려도 항상 |20mm| 가 나온 것이다. 도달
불가가 아니라 자 눈금이 틀렸다. 고치니 0.0mm.

**(e) 플랜지 접촉 기본값 복원 [§18-5 → §20-3 → 확정].** §20-3 이 이미 "기본값을
켬으로 되돌리는 것이 맞다"고 적었는데 `LAYOUT_FROM_STEP` 이 기본 1로 승격되면서
결과적으로 꺼진 상태가 됐다. `LEFTWALL_FLANGE_CONTACT` 기본값을 1로 고정했다.
실측 대조: ON 이면 벽이 −12.79mm 에 서고 OFF 면 −14.53mm 까지 들어간다.

### 21-6. Phase 12 — 회수장치 이송·고정

사용자 지시로 분쇄 후 회수장치까지 붙였다.

- **회수장치를 IPC 에 넣어야 했다.** `needs_coup=False` 라 턱이 봉투를 통과했다.
  그냥 넣으면 uipc 가 죽는다 — 턱 6개 링크의 충돌 메시가 **전부 non-watertight**
  다(실측). 런타임에 **볼록껍질**로 갈아끼운 MJCF 사본을 굽고 그 6개만
  `coup_links` 로 넣는다(`_prepare_recovery_mjcf`).
- **샤프트 `회전_31`: q=0 닫힘 / q=π 열림.** 슬라이더 안 라쳇 때문에 닫을 때는
  역회전이 아니라 **π→2π 전진**이다. 그리고 **기구학 구동이어야 한다** —
  `control` 로 주면 180→360 지령에 33,153°(92바퀴) 폭주한다. 회수장치도 크러셔와
  같은 폐루프이기 때문이다.
- **경로는 Genesis 내장 `plan_path`(RRTConnect)로 푼다**(`rigid_entity.py:3467`,
  `ignore_collision=False` 기본). 손으로 만든 경유점은 self-collision 을 보지
  않아 봉투를 30° 흔들었다 — 플래너로 바꾸니 2.0° 다.
- **결과**: 해제 후 낙하 −4.2mm 로 회수장치가 봉투를 잡는다. 다만 투입 자세가
  23° 기울어 있어 품질은 미해결이다. yaw 90°(실링 정렬)와 낮은 tilt 가 현재
  트레이드오프 관계다.

**석션V1**: 초기 자세를 LSM 액추에이터 후방 +100mm, 그리퍼 −50mm(개방)로 뒀다.
좌우 그리퍼는 MJCF `equality polycoef="0 1 0 0 0"` 으로 **R = L 1:1 mimic** 이고
축이 반대(L +Y / R −Y)라 같은 q 가 대칭으로 벌린다 — 실기 rack&pinion 과 같다.
`range="-0.05 0"` 이라 −50mm 가 완전 개방이다.

### 21-7. 기본값 승격

환경변수 없이 그냥 돌려도 검증된 구성이 되도록 옮겼다.

| 항목 | 이전 | 현재 | 근거 |
|---|---|---|---|
| `LAYOUT_FROM_STEP` | 0 | **1** | §19 실설계 배치가 검증된 구성. 0 이면 오늘 고친 것 대부분이 적용되지 않는다 |
| `WALL_KINEMATIC` | 0 | **1** | §21-5(b) — 힘 균형점이 없어 유일한 방법 |
| `LEFTWALL_FLANGE_CONTACT` | 0(실배치) | **1** | §21-5(e) |
| `CRANK_KV_SPIN` | 15 | **5000** | 가설 3 기각. 원래 값이 옳았다 |

잠복 버그도 하나 터졌다 — `_q_cmd` 가 velocity 분기 안에서만 초기화돼
`WALL_KINEMATIC=1 + CLAMP_MODE=position` 조합에서 `UnboundLocalError` 였다.
기본값 승격으로 그 조합이 처음 실행되면서 드러났고, 모드와 무관하게 초기화하도록
올렸다.

### 21-9. [미해결] 플랜지 접촉을 켜니 그 뒤 구간이 깨졌다

§21-5(e) 대로 `LEFTWALL_FLANGE_CONTACT` 를 1로 되돌린 뒤 **처음으로 전 시퀀스를
기본값만으로 돌린 결과, 봉투가 크러셔에 남았다.** 팔만 회수장치로 갔다.

```
clamp    wall = -12.21mm   (플랜지면 잔여 0.29mm / 본체면 5.29mm)  <- 의도대로
crush    wall = -16.97mm   <- 분쇄 중 4.76mm 더 파고듦
unclamp  wall = -16.82mm   <- +6mm 지령인데 0.15mm 밖에 안 열림
extract  bag_com x=0.2131, finger_z 0.2868  <- 봉투가 안 따라 나옴
투입     수평오차 396.7mm  <- 봉투는 크러셔 슬롯에 그대로
```

**압착 자체는 플랜지 ON 이 옳다** — 벽이 −12.21mm 에 서고 잔여도 §20-3 수치로
돌아왔다. 깨진 것은 **그 뒤 구간**이고, 원인이 둘 겹쳤다.

1. **분쇄 구간이 벽 위치를 유지하지 못한다.** `_wall_hold` 를 매 스텝 지령하는데도
   −12.21 → −16.97mm 로 밀린다. §21-4 의 미확정 항목과 같은 자리다 — 힘제어로
   "이미 도달한 위치를 유지"하는 것이 실제로는 유지가 안 된다.
2. **`unclamp` 가 실제 위치가 아니라 `_wall_hold` 에서 램프를 시작한다.** 벽은
   −16.97mm 에 있는데 −12.2mm 를 기점으로 올리므로, 박힌 벽을 열 지령이 사실상
   안 나간다.

**그리고 이 런은 `verdict=PASS` 로 찍혔다.** 판정이 삽입/붕괴만 보고 **봉투가
회수장치까지 갔는지는 안 보기 때문**이다. §20-7 이 "verdict 는 압착을 측정하지
않는다"고 정정했는데 이송에 대해서도 같은 한계가 있다. 조용히 실패하는 판정은
없느니만 못하므로 게이트가 필요하다.

**조치**: 회수장치 투입 공정이 1순위이므로(§21-8) 그 작업을 막지 않도록
`LEFTWALL_FLANGE_CONTACT` 기본값을 **잠시 0으로 되돌렸다**. §20-3 의 결론을
뒤집는 것이 아니라 작업 순서상의 임시 조치다. 위 두 가지를 고치면 1로 돌아간다.

### 21-10. 회수장치 투입 성립 — 원인은 **보간 비율**이었다 (2026-09-01)

투입 자세가 27~30도 기울던 문제가 풀렸다. 원인은 자세도 속도도 아니고
**`plan_path` 웨이포인트 수와 이송 스텝 수가 같았던 것**이다.

```
plan_path(num_waypoints=600) + N_TO_RC=600
  -> run_arm_path 가 스텝당 웨이포인트를 하나씩 소비. 보간이 아예 안 일어난다.
  -> RRT 경로가 길어지면 한 스텝의 관절 이동이 커져 봉투를 채찍질한다.
```

진입 고도를 올려 RRT 경로가 길어지자 그대로 터졌다 — 핑거가 z=1.23m 로 튀고
FEM 이 발산해 수평오차 64,759mm 가 찍혔다. **웨이포인트 120 / 이송 1800 스텝**으로
15배 보간되게 고치자 해결됐다.

| | 이전 | 이후 |
|---|---|---|
| tilt | 29.6° | **7.3°** |
| 수평오차 | 12.8mm | **4.4mm** (설계 여유 실링당 4.5mm) |
| bag_bottom 오차 | +56mm | **−3.6mm** |
| 해제 후 낙하 | −3.1mm | **−0.0mm** |

해제 후 tilt 가 7.3° → 2.7° 로 **오히려 펴진다** — 턱이 양쪽 실링을 제대로 물어
봉투를 바로잡는다는 뜻이다. 한쪽만 물던 이전과 다르다.

**스윕 결과(`sweep_recover_20260831.py`)**: 네 축 중 **yaw 만 유의미**했다.

| 축 | 결과 |
|---|---|
| yaw | **90°가 135° 대비 수평오차 절반**(4.3~4.4 vs 8.3~8.8mm), 2쌍에서 재현 |
| 진입 고도 | 60 vs 80mm → 0.1mm 차이. 무관 |
| 하강 스텝 | 600 vs 900 → 0.1mm 차이. 무관 |
| 하강 웨이포인트 | 9 vs 21 → **소수점까지 동일**. 무관 |

실링 정렬 자세(90°)가 성능까지 더 좋다는 것이 확정됐다. 이전에 "90°가 나쁘다"고
본 것은 **채찍질의 부산물**이었다 — 보간을 고치자 뒤집혔다.

**폐기: 턱 상단 역산 진입.** 봉투가 핑거 아래 82mm 라 턱 상단(0.3005)을 넘기려면
핑거가 0.3825 이상이어야 하는데 그 (x,y)에서 팔이 닿는 높이가 0.40 언저리다
(여유 20mm). CLEAR_Z=40 이면 IK 오차 17.2mm 로 도달 불가다. 애초에 넘길 필요도
없었다 — 흔들림의 원인이 고도가 아니었기 때문이다. `RECOVER_CLEAR_Z` 기본값 0.

**실패 게이트를 자리마다 붙였다.** 오늘 겪은 실패 세 종류가 전부 조용히
지나갔었다(수평오차 396.7mm 인데 verdict=PASS, 64m 발산 수치가 스윕 표에 혼입,
17.2mm 도달오차로 강행). 이제 각각 중단하고 이유를 찍는다.

```
unclamp  벽이 5mm 이상 안 열리면      중단
extract  봉투가 핑거 상승의 50% 미만  중단
rcset    bag_com 3m 초과 / non-finite 중단
IK       진입·물림 오차 15mm 초과     이송 생략
```

### 21-11. [미해결] 플랜지 ON 은 두 갈래 다 막혔다

§21-9 의 두 처방을 모두 시도했고 **둘 다 실패**했다.

| 시도 | 결과 |
|---|---|
| `unclamp` 램프 기점을 실제 벽 위치로 | −16.94 → −16.80mm, **열린 양 +0.14mm** |
| 분쇄 중 50스텝마다 벽 `set` | **크랭크 사망**(0.02 RPM), 벽도 −15.74mm 로 여전히 안 열림 |

기점은 원인이 아니었다. 그리고 두 번째가 §21-4 의 미확정 메커니즘에 단서를 준다 —
**`set` 한 번의 영향이 그 스텝에서 끝나지 않는다.** 1/50 로 줄여도 크랭크가
3000스텝 내내 0.018°/스텝으로 일정해 매 스텝 `set` 과 구별되지 않는다. 구속 솔버
웜스타트가 한 번 날아가면 회복에 50스텝 이상 걸린다는 뜻이다. 사용자가 제안한
"set 지령은 한 번만" 방향은 N=50 에서 기각이다.

대조하면 플랜지가 벽을 **끌고 들어간다**는 것이 보인다.

```
플랜지 OFF   clamp -14.53  ->  crush -12.82   (분쇄 중 오히려 빠져나온다)
플랜지 ON    clamp -12.21  ->  crush -16.94   (분쇄 중 4.7mm 파고든다)
```

§18-5 의 "플랜지가 마주보는 면 없이 허공을 누른다"와 같은 자리다. 압착 순간에는
문제가 없지만 분쇄 중 반복 하중이 걸리면 벽을 계속 끌어당긴다. **`unclamp` 를
고칠 문제가 아니라 분쇄 중 벽 유지를 다른 방식으로 풀어야 한다.**

기본값은 `LEFTWALL_FLANGE_CONTACT=0` 을 유지한다 — 전 시퀀스 완주가 1순위이고
(§21-8), 플랜지 OFF 로는 완주하기 때문이다.

### 21-8. 남은 과제

**우선순위는 `full_workflow.py` 가 전 시퀀스를 완주하는 것이다. 아래는 전부
차순위다.**

1. ~~회수장치 투입 자세~~ **[해결 §21-10]** — tilt 7.3°, 수평오차 4.4mm,
   낙하 0.0mm. 전 시퀀스가 환경변수 없이 완주한다.
1-b. **플랜지 접촉 ON 복귀 [미해결 §21-11]** — 두 처방 다 실패했다. 분쇄 중
   벽 유지를 `set` 이 아닌 다른 방식으로 풀어야 한다. 남은 후보: 벽 DOF 에
   armature 를 크게 걸어 끌림을 줄이는 것, 또는 플랜지를 고정 프레임 쪽 링크로
   분리해 벽과 함께 움직이지 않게 하는 것(§18-5 가 원래 처방한 CAD 재export).
2. `set`/`control` 혼용이 정확히 무엇을 깨는지(§21-4 미확정).
   `_is_forward_vel_updated` 를 매 스텝 False 로 되돌려 보면 갈린다.
3. 정제 분쇄량을 못 잰다. FEM 요소가 붕괴해 두께가 튄다(Y 가 0.05~0.39mm 로
   찍혔다가 복구). 분쇄 지표로 쓸 수 없다.
4. 압착력 측정 — §21-5(b) 대로 이 봉투로는 불가능하다. 재려면 봉투 모델 자체를
   바꿔야 한다.


---

## 22. 흡착 개구 — IPC 커플러는 FEM 정점 구속을 안 읽는다 (2026-09-02)

### 22-0. 한 줄 요약

**Genesis 에는 "특정 정점 집합을 당기는(흡착)" 기능이 IPC 씬에서 구현돼 있지
않았다.** uipc 에는 있는데 Genesis 가 배선을 안 했다. 몽키패치로 이었다.
패치 자체의 상세는 `docs/GenesisPatch.md` 에 따로 둔다.

### 22-1. 증상

봉투 입구를 흡착컵으로 벌리려 했으나 두 갈래로 다 막혔다.

| 구속 방식 | 결과 |
|---|---|
| `set_vertex_constraints(is_soft_constraint=True)` | **아무 일도 안 일어남** |
| `set_vertex_constraints(is_soft_constraint=False)` | 그 정점만 순간이동, 이웃이 안 따라와 메시가 찢김 |

사용자가 처음부터 지적한 "FEM weld 는 정말 한 점만 당겨지는 느낌"이 이것이다.

### 22-2. 변인통제

**소프트 구속은 강성에 전혀 반응하지 않는다.** 1만 배를 훑어도 소수점까지 같다.
...
**재료 파라미터도 아니다.** `CLOTH_E` 4e5→4e8(1000배), `dt` 5e-3/2e-3,
`d_hat` 1e-4/1e-2(100배), `bending` 50/400 을 다 훑어도 봉투 z 길이가 안 변한다.

**셸은 정상이다.** 구속을 아예 빼고 자유낙하시키면 봉투가 제 형상(64×90×6mm)을
유지한 채 바닥에 눕는다. 즉 범인은 재료가 아니라 구속이다.

공식 예제 파라미터(`dt=2e-3, d_hat=0.01, E=1e6, bend=50`)로도 돌려봤는데 **더
나빴다** — `d_hat=0.01` 은 10mm 안의 모든 걸 밀어내는데 이 그리퍼는 q=0 에서 두
컵 면이 맞닿는 구조라 항상 배리어 위반이고, IPC 가 조를 한계 밖 +33mm 까지
밀어냈다. 공식 예제는 컵이 없는 자유 낙하 천이라 이 조건이 없다.
`d_hat=1e-4` 는 이 씬에 필요한 값이었다.

### 22-3. 원인

`ipc_coupler/coupler.py` 전체에 **FEM 정점 구속을 참조하는 코드가 한 줄도 없다.**
거기 있는 constraint 는 전부 강체 링크용이다. IPC 를 켜면 천은 uipc 가 스텝하는데
`set_vertex_constraints` 는 Genesis 자체 FEM 솔버 버퍼에만 쓴다 — 아무도 안 읽는다.

즉 Genesis 에 구현이 없는 게 아니라 **FEM 정점 구속의 IPC 판이 없다.**
Genesis 메인테이너도 흡착에 대해 "Not yet. Suction cup requires adding an
additional constraint, which should be easy to add" 라고 답했다(Discussion #87).

### 22-4. 해결

uipc 에 이미 있는 `SoftPositionConstraint` 를 천 메시에 배선한다
(`utills/fem_ipc_workarounds.py::patch_ipc_vertex_attach`).

| 방식 | 봉투 bbox (원래 64 × 6 × 90mm) |
|---|---|
| 하드 핀 | z 470~545mm — 찢김 |
| 소프트 | 무반응, 바닥 |
| **SoftPositionConstraint** | **64.0 × 6.1 × 90.0mm, 중심 오차 0.0mm** |

애니메이터 콜백도 필요 없었다 — 슬롯에 직접 쓴 값이 uipc 솔브까지 전달된다.

### 22-5. 같이 고친 것 셋

**(a) 측면 필터.** 봉투 두께 6mm(면이 ±3mm)에 컵 반경 7.5mm 라, 반경만으로 고르면
각 컵이 반대쪽 면 정점까지 잡아 서로 상쇄된다.

    컵당 16개(최근접 1.6mm) -> 개구  2.6mm
    컵당  7개(최근접 3.4mm) -> 개구 56.1mm

접촉면 기준 자기 쪽만 고르니 런마다 널뛰던 결과가 재현된다.

**(b) 개루프 → 폐루프.** 종전에는 조인트 명령값 `dy` 만큼 기억한 좌표를 평행이동
했다(`side` 부호를 손으로 지정). 컵이 명령대로 안 가면 목표가 틀리고 회전은 아예
반영이 안 된다. 파지 순간의 **컵 로컬 좌표**를 저장해 매 스텝 컵의 실제 자세로
되돌린다. `side` 도 `dy` 도 필요 없어진다.

**(c) 개방 행정 50 → 20mm.** 짧게 열수록 잘 따라온다(사용자 지시).

    행정 50mm  개구 +66.7mm / 컵 이동 85mm   follow 0.785
    행정 20mm  개구 +19.7mm / 컵 이동 20mm   follow 0.983

### 22-6. 흡착 위치 스윕 (`docs/suction_zsweep.png`)

봉투 입구는 로컬 z=+45mm 의 테두리 34정점이다 — **경계 에지 위상**으로 확인했다
(바운딩박스로는 안 보인다. 5-panel 서피스 모델이라 슬라이스 단면이 균일하다).

| 흡착 위치(테두리 아래) | 개구 | follow |
|---|---|---|
| 5mm | 72.6mm | 0.769 |
| **15mm** | 66.7mm | **0.785** |
| 25mm | 69.0mm | 0.746 |
...
5~25mm 는 사실상 같고 45mm(봉투 중간)를 넘으면 급락한다. 그 너머는 입구가 아니라
몸통을 무는 셈이다. 봉투 형상은 다섯 런 모두 64.0×6.2×90.0mm 로 유지된다.

**주의**: 점이 5개, 런당 1회다. 5~25mm 안에서의 순위는 재현 편차 안에 있다.

---

## 23. `full_workflow.py` 이식과 그 과정에서 나온 것들 (2026-09-02~03)

### 23-1. 봉투 형상 고정 마스크가 축을 잘못 짚고 있었다

    [bag dbg] 선택 714개 (바닥 102 + 측면 714), y폭 6.0mm     <- 고치기 전
    [bag dbg] 선택 246개 (바닥 102 + 측면 168), x폭 64.0mm    <- 고친 후

측면 마스크가 `y` 에 8mm 밴드를 걸고 있었는데 `BAG_EULER=(90,0,90)` 에서 y 는
**두께 6mm** 축이다. 6mm 축에 8mm 밴드면 전부 선택된다. 주석의 "입구는 자유"가
**한 번도 성립한 적이 없었고** 봉투 714정점이 통째로 고정돼 있었다. 폭 64mm 는 x 다.

### 23-2. 핑거와 회수장치가 안 부딪히는 이유

`enable_rigid_rigid_contact=False` 다. 켜면 빌드가 죽는다 — 전역 스위치라 관절로
이어진 로봇 링크끼리도 검사해서 초기 관통으로 걸린다("World is not valid").
회수장치 `_col` geom 은 `contype` 기본값(=1)이라 Genesis 자체 강체 솔버에서는
충돌 대상이어야 맞는데, **거기서 왜 안 잡히는지는 미확인.**

### 23-3. 매니퓰레이터가 헐렁하게 진동한 이유

`run_arm` 이 끝나면 아무도 로봇에 명령하지 않는다. Phase 13 의 3,000스텝 동안
중력에 흔들렸다. 흡착 루프마다 `set_dofs_position(Q_GRASP)` +
`set_dofs_velocity(None)` 으로 붙잡아 해결.

### 23-4. 흡착장치 배치 격차 — **미해결**

    컵 중점        (-58.8, -192.8, 289.5) mm
    봉투 bbox   x[-65.9,-52.5]  y[-307.4,-237.2]  z[195.5,289.9] mm

- **x** 이미 맞음(컵이 닫히는 방향이 world x, 봉투 x 중심 -59.2)
- **z** 이미 맞음(봉투 상단 289.9 vs 컵 289.5). 배치를 5mm 올려 더 맞췄다
  (`RC_BITE_DEPTH` 10 → 5mm, 실측 착지 0.1969 → 0.2019).
- **y 44~53mm 모자람.** 스테이지 100mm 를 다 써도 안 닿는다.

`SUCTIONV1_POS` 가 회수장치 턱에서 역산되지 않고 STEP 평면 좌표로만 잡혀 있다
(993행 주석에 "사용자 확인 필요"가 이미 적혀 있었다). **실기 배치 기준값 필요.**

### 23-5. 조 진동 — 컵끼리의 자가충돌이었다

**증상.** 조 간격이 16mm 아래로 좁혀지는 순간부터 튄다. 그 전 600스텝은 오차
0.35mm 이내로 완벽하게 따라온다.

    close  601/1000  cmd jaw=-15.79 | act L=-16.13  R=-16.13   stage= 0.00   정상
    close  901/1000  cmd jaw= -0.42 | act L=-10.26  R=-10.30   stage=+1.89   9.6mm 튐

좌우가 0.3mm 안쪽으로 **같이** 움직이니 미믹 문제는 아니다. 그리고 명령이 전혀
안 바뀐 **스테이지까지 ±1.9mm 흔들린다** — 조에 걸린 힘이 스테이지로 전달된다.

**변인통제(사용자 지시).** 가설이 둘이었다 — (a) 컵끼리 자가충돌, (b) 0 까지 닫는
구동 자체. 가르는 방법은 조가 0 에 못 가게 **디스크립션 단에서 막는 것**이다.
MJCF 사본을 만들어 조 range 를 `-0.05 ~ -0.003` 으로 제한했다
(`_prepare_suction_mjcf()`, `SUCK_JAW_MIN_MM`). 원본 에셋은 안 건드린다.

| | 901스텝 명령 | 실제 L / R | 스테이지 |
|---|---|---|---|
| 리밋 없음(0mm 까지) | -0.42mm | **-10.26 / -10.30mm** | **+1.89mm** |
| 리밋 -3mm | -3.03mm | -3.00 / -3.06mm | +0.01mm |

**(a) 가 원인이다.** 구동 방식·게인·미믹을 하나도 안 바꾸고 조가 0 에 못 가게
막기만 했는데 추종 오차가 **9.6mm -> 0.03mm**, 스테이지 흔들림이
**1.89mm -> 0.01mm** 로 떨어졌다. 조 간격 6mm(q=-3mm) 아래에서 두 컵 면이 서로
파고들고, 접촉 배리어와 PD 가 싸우던 것이다.

**이 리밋은 임시방편이 아니라 물리적으로 옳은 값이다.** 실기에서도 두 흡착컵이
서로 맞닿을 때까지 닫히지 않는다 — 사이에 봉투가 있다. 봉투 두께 6mm 를 감안하면
`-3mm` 가 그 자리다. `SUCK_JAW_MIN_MM=0` 을 주면 종전 거동으로 돌아간다.

**왜 SuctionV1_only 에서는 안 나왔나.** 거기서는 컵 사이에 봉투가 있어 조가 봉투에
받쳐 멈췄고, 자가충돌 구간(-3mm~0)에 애초에 들어가지 않았다. full_workflow 에서는
배치 격차(§23-4) 때문에 컵 사이가 비어 있어 끝까지 닫히려 했다. **즉 이 진동은
배치 격차의 2차 증상이기도 하다** — y 가 해결되면 봉투가 받쳐 자연히 안 난다.

### 23-6. 공정 단계별 계산 비용 (`docs/workflow_cost.png`)

전 공정 930.5s / 14,142스텝(영상 없음).

| 구간 | 벽시계 | 스텝 | ms/스텝 |
|---|---|---|---|
| 파지 → 슬롯 삽입 | 143s (15%) | 1,930 | 74 |
| 클램프 | 44s (5%) | 962 | 46 |
| 압착 15초 | 130s (14%) | 3,000 | **43** |
| 언클램프 → 투입 | 279s (30%) | 4,200 | 66 |
| 회수장치 잠금 | 22s (2%) | 450 | 49 |
| 복귀 + 해제 | 31s (3%) | 600 | 51 |
| **흡착 전진 + 파지** | **282s (30%)** | 3,000 | **94** |

**압착이 스텝당 가장 싸고 흡착이 가장 비싸다(2.2배).** 압착이 오래 걸리는 건 스텝이
많아서일 뿐이다. 흡착이 왜 비싼지는 미확인.

**영상 유무 차이**(같은 코드/설정/스텝 수):

    영상 없음   930.5s   65.8 ms/스텝
    영상 있음  1103.7s   78.0 ms/스텝     +18.6%

카메라 3대가 스텝당 12.2ms. 스윕은 `NO_VIDEO=1` 로 20% 아낄 수 있다.

**GPU**: RTX 5080, 사용률 37%, 7.0/16.3 GiB. GPU 병목이 아니다 — 스텝마다 Python
루프가 개입하는 직렬 구간이 GPU 를 놀린다.

### 23-7. 남은 것

1. **`SUCTIONV1_POS` y 배치** — 실기 기준값 필요. 이게 풀려야 흡착이 성립한다.
2. ~~조 진동~~ — 해결(§23-5). 컵 자가충돌이었고 조 하한 -3mm 로 막았다.
3. **컵 축 오독** — 두 컵 링크 원점이 z 로 36mm 어긋나 있어(원점이 컵 면이 아님)
   `_ax = (pR-pL)` 로 뽑은 축이 틀렸다. 조인트 변위에서 뽑아야 한다. **미수정.**
4. **핑거-회수장치 강체 충돌**(§23-2) 미확인.
5. **야간 런 A/B 비교 무효** — 마스크 축 수정(§23-1)이 두 경로에 공통 적용돼
   SPC 효과와 분리가 안 됐다. 다시 재려면 마스크를 고정하고 SPC 만 켜고 꺼야 한다.

---

## 24. 낟알(IPC Particle)을 `full_workflow.py` 에 배선 — 변인통제 기록 (2026-09-08)

§9 조합10 이후 파우더 표현은 uipc 네이티브 `Particle` 로 굳었고(`GrainIPCCoupler`),
그동안은 `Powder_flip_test/ipc_grain_coupler.py TEST_MODE=bag` 에서 봉투와 낟알
둘만 놓고 검증해 왔다. 이번에 그걸 전 공정 스크립트로 옮겼다.

### 24-1. 배선 — 낟알은 build 전에 등록해야 하는데 봉투는 공정 내내 움직인다

IPC 씬은 `world.init()` 이후 지오메트리 추가가 안 된다. 그런데 낟알을 부을
자리(봉투 입구)는 흡착이 끝난 **마지막 스테이지**에서야 정해진다. 그래서:

1. `scene.build()` 전에 낟알을 등록하되 좌표는 **대기 지점**(회수장치 바로 위
   z=900mm)으로 준다
2. 봉투 형상 고정에 쓰는 것과 **같은 메커니즘**(uipc `SoftPositionConstraint`,
   §22)을 낟알 pointcloud 에도 걸어 그 자리에 붙잡아 둔다
3. `grain` 스테이지에서 실측한 입구 위로 목표를 옮기고, 도착한 뒤 구속을 푼다

커플러는 `gs.Scene()` 직후 `scene._sim._coupler` 를 서브클래스로 갈아끼운다
(Genesis 는 `coupler_options` 의 타입만 보고 내부에서 기본 `IPCCoupler` 를 만든다).
`patch_ipc_vertex_attach` 는 기반 클래스 메서드를 패치하므로 서브클래스가 그대로
물려받는다.

**이송을 대각선으로 하면 안 된다.** 처음엔 대기 지점에서 투입점까지 직선으로
0.85m 를 0.5초에 옮겼는데, 낟알 무리가 로봇과 크러셔를 관통해 지나갔고
`two_way_coupling` 이라 그 반력이 Genesis 강체 솔버로 들어가 죽었다:

    genesis.GenesisException: Invalid constraint forces causing 'nan'.

**수평(대기 고도, 씬이 비어 있는 높이) → 수직 하강** 두 구간으로 쪼개고, 고정
시간이 아니라 **최대 속도**(`GRAIN_APPROACH_V`, 기본 0.5 m/s)로 잡아 해결했다.
거리는 스테이지 조합에 따라 크게 달라지므로(회수장치 vs 크러셔 슬롯) 시간을
고정하면 거리가 멀 때 낟알이 그대로 쓸려간다.

또 하나 — **대기·이송 중에는 낟알을 그리지 않는다**(`_grain_visible`). 그 구간은
낟알을 SPC 로 붙잡고 옮기는 중이라, 그리면 강체 덩어리가 공중에 굳은 채
미끄러지는 그림이 돼 "물리가 멈췄다"로 오독된다(실제로 그렇게 읽혔다).

### 24-2. 담김 판정식이 허수를 냈다

처음 판정은 "봉투 바닥 z 보다 10mm 아래로 가면 누출" 이었다. `STAGE=grain` 스모크런
에서 **담김 500/500, 누출 0%** 가 나왔는데, 그 런은 앞 단계를 건너뛰어 봉투가
납작하게 누워 있었다(`width 100.8mm / height 6.0mm`, 정상은 6.0 / 90.0). 바닥보다
위이기만 하면 참이 되니 자동으로 100% 다.

바닥 z 뿐 아니라 **상단(입구) z 와 xy 외곽까지, 매번 현재 봉투 자세로 다시 재서**
본다(`_grain_inside`). 담김 / 누출(바닥아래) / 잔류(입구위) / 옆(xy밖) 네 갈래로
나온다. xy 를 넣은 이유는 높이만 맞고 봉투 **옆에** 있는 낟알이 담김으로 세어지는
구멍이 남기 때문이다.

### 24-3. 낟알끼리는 한 번도 닿지 않는다 — 그런데 이건 버그가 아니다

최종 배치의 최근접 이웃이 **정확히 `2R + d_hat`** 에 굳는다. 실제 접촉거리 `2R` 인
쌍은 단 하나도 없다.

| 환경 | 초기 최소간격 | d_hat | 최종 최근접 | = 2R+d_hat |
|---|---|---|---|---|
| probe (봉투) | 2.0mm | 0.5mm | 1.499mm | 1.500mm |
| full_workflow | 1.4mm | 0.1mm | 1.100mm | 1.100mm |

초기 배치 탓이 아니다 — 두 경우 다 초기 간격보다 **더 가까워졌다가** 각자의
d_hat 을 따라 멈췄다. 솔버가 덜 수렴해서도 아니다 — `newton_tolerance` 를 50배
조이면 오히려 그 값에 **더 정확히** 붙는다(2R+d_hat 보다 가까운 쌍 42 → 0).

이유는 배리어 강성 대 낟알 무게의 비다. `contact.adaptive.init_kappa = 1e9` 인데
R=0.5mm/rho=1500 낟알의 무게는 **7.7 μN** 이라, 자기 무게로 배리어를 눌러 들어가는
양이 나노미터 수준이다. 즉 **낟알은 반지름이 `d_hat/2` 만큼 부푼 구처럼 행동한다.**
접촉력은 정상적으로 전달되므로 고칠 대상이 아니라 관리할 대상이다.

> **`d_hat / R <= 0.2` 를 지켜라.** d_hat=R 이면(probe 초기 설정) 유효 반지름이
> 1.5배가 되어 부피가 3.4배로 부푼다. 게다가 임계가 개체별 이진이라 "일부는 잘
> 떨어지고 일부만 이상하게 굳는" 것처럼 보여 전역 감쇠로 오진하기 쉽다.

### 24-4. IPC Particle 은 이 환경에서 정상이다

"Genesis 강체와 IPC 가 공존하는 조건에서 낟알이 제대로 도는지부터 확인하자"는
지시로 `GRAIN_TEST=freefall` 을 넣었다. 봉투도 이송도 없이 대기 지점에서 그대로
놓고 z(t) 를 자유낙하와 대조한다.

| 항목 | 실측 | 기대 |
|---|---|---|
| 자유낙하 가속도 | **9.806** m/s² | 9.810 |
| 낙하 중 무리 z 표준편차 | 11.608 → 10.816mm | 불변(유령 접촉 없음) |
| 강체(플레이트) 착지 높이 | **0.600mm** (최소 0.599) | R + d_hat = 0.600 |
| 천 위 착지 간격(최하층 중앙값) | **0.89mm** | 0.600 + 메시오차 ≤2mm |
| 압밀 배위수 | 5.24 | 조밀충전 ~6 |
| 담김/누출/잔류/옆 | 500 / 0 / 0 / 0 | — |

낙하도, 강체 접촉도, 천 접촉도, 낟알끼리 압밀도 전부 정상이다.

**부양은 없다.** "낟알 213mm vs 봉투 최하단 201.7mm" 의 12mm 간격은 봉투 **바깥
실링선과 내부 바닥의 차이**였다. 천까지의 거리를 직접 재면 sub-mm 다.

이 절에서 **오진을 두 번 했고 둘 다 계측 설계 잘못이었다**:

1. **"자유낙하 가속도 -3.93 m/s², 이상"** — 대기 지점 아래에 아무것도 없다고
   가정하고 1초 창 전체를 적합했는데, 베이스 플레이트와 회수장치가 있어서
   t≈0.39s 착지가 섞였다. 접촉이 없는 동안 무리의 z 표준편차가 상수라는 성질로
   접촉 시점을 자동 검출해 그 앞까지만 적합하도록 고쳤다.
2. **"체적률 3~8%, 더미가 아니라 성긴 비계"** — 봉투 내부를 bounding box 로 잡아
   부피를 크게 과대평가했다. 파우치는 상자가 아니다. 기하에 안 휘둘리는
   **배위수**로 다시 재니 4.3~5.2 로 정상 충전이었다.

계측 지표를 기하 가정 위에 세우면 이렇게 틀린다. 배위수처럼 국소적이고 기하
독립인 지표를 우선한다.

### 24-5. 솔버 노브 — 셋 다 원인이 아니었다 (단, probe 환경 한계)

| 조건 | 배위수 | 더미 높이 | 비고 |
|---|---|---|---|
| 기준 | 3.72 | 7.4mm | |
| `newton_tolerance=1e-3` | **4.26** | 4.9mm | 유일하게 이득 |
| `contact_eps_velocity=1e-4` | 3.60 | **88.5mm** | 크게 악화 |
| 마찰 μ 0.6 → 0.3 → 0.1 → 0.0 | 4.27 → 4.13 → 3.82 → **3.01** | | 현행 0.6 이 최선 |

`eps_velocity` 는 IPC 마찰이 Coulomb μN 으로 포화되는 속도 문턱이라, 낮추면 느리게
움직이는 낟알이 곧바로 완전 마찰을 받아 닿은 자리에 붙는다 — 낟알이 88.5mm 에
늘어붙어 체적률이 무너진다. 마찰은 낮출수록 누출만 늘고 배위수는 떨어진다.

> **이 스윕은 판단 근거로 쓰지 마라.** probe(`TEST_MODE=bag`)에는 Genesis 강체가
> 아예 없어서, IPC 와 Genesis 두 솔버가 같이 도는 실제 조건을 대변하지 못한다.
> 이후 변인통제는 전부 `full_workflow.py` 기준으로 한다(사용자 지시).
> 드라이버: `probe/drivers/grain_fullwf_sweep.sh`

### 24-6. `newton_tolerance` 가 무엇인가

Genesis `IPCCouplerOptions.newton_tolerance` → libuipc `config["newton"]["velocity_tol"]`
(`ipc_coupler/utils.py:124`), 기본값 **0.05 m/s**.

libuipc 적분기는 BDF1(후진 오일러, `integrator.type == "bdf1"`)이라 "새 위치는 새
위치에서 평가한 힘과 앞뒤가 맞아야 한다"를 풀어야 한다. IPC 는 이걸 최소화 문제
(관성 + 탄성 + 접촉 배리어 + 마찰)로 바꾸고 **뉴턴 반복**으로 푼다 — 국소 2차
근사 → 선형 시스템 한 번 풀이(`fused_pcg`) → line search → 반복. `velocity_tol` 은
그 보정량이 얼마나 작아지면 멈출지의 문턱이고, 보정량을 dt 로 나눠 속도로 잰다.

dt=5ms 에서 0.05 m/s ≈ **0.25mm** — 낟알 반지름(0.5mm)의 절반이다. 즉 기본값은
"낟알을 반지름 절반만큼 더 밀고 싶은데 여기서 멈춰도 된다"는 뜻이라 낟알 크기
대비 성기다. (×dt 환산은 속도형 수렴판정의 표준 해석이고, libuipc 가 컴파일
바이너리라 검사식 자체를 소스로 확인하지는 못했다.)

전 구간 런 실측(N=500, 영상 켬):

| | NEWTON_TOL 기본 0.05 | 1e-3 |
|---|---|---|
| 더미 높이 | 28.7mm | **12.3mm** |
| 배위수 | 5.16 | 5.24 |
| **전 공정 소요** | 2055s (34분) | **2951s (49분, +44%)** |

낟알이 훨씬 잘 눌러앉지만 **낟알 구간만이 아니라 전 공정이 44% 느려진다** —
전역 솔버 설정이기 때문이다. 기본값으로 박으려면 낟알 없는 기존 공정 결과가
안 바뀌는지부터 확인해야 한다.

### 24-7. 남은 것

1. **`N_GRAINS` 기본값** — 현재 500 = 0.393g(rho=1500). 프로젝트 기준 장입량은
   소금 2g = 1768알(rho=2160). 비용 스케일 미측정.
2. **`NEWTON_TOL=1e-3` 을 기본값으로 박을지** — 전 공정 +44% 이고, 낟알 없는
   런에 영향이 있는지(압착 벽 반력·봉투 거동) **미확인**. 1e-2 로도 이득이
   나오는지도 미측정.
3. **압밀이 수렴했는지** — 정착 2초로 끊었다. 더 주면 더 눌러앉는지 미확인.
4. **판정 기준면** — 봉투 최하단 정점(바깥 실링선)과 내부 바닥이 12mm 차이 난다.
   "담김" 을 어느 면 기준으로 잴지 정해야 한다.
5. probe 에서 관측된 **공중에 뜬 26알**(이웃이 1.5~9.9mm 떨어져 아무것도 안 닿음)
   은 미규명. 다만 full_workflow 에서는 잔류 0 이라 우선순위는 낮다.

1~3 은 `probe/drivers/grain_fullwf_sweep.sh` 7조로 측정 중이다.

---

## 25. 봉투가 왜 덜컥거리는가 — 파우더였다 (2026-09-10)

`full_workflow.py` 의 파지→삽입 구간에서 봉투가 이산적으로 덜컥인다는 지적이
있었다. 전 구간은 한 조에 45~60분이라 변인통제가 불가능해서, **봉투 + 파우더 +
잡아주는 것**만 남기고 10초를 흔드는 격리 프로브를 만들었다
(`probe/probe_bag_shake.py`, 한 조 2~10분).

### 25-1. 지표를 먼저 고쳐야 했다

처음 쓴 지표는 **COM 가속도의 부호 반전율**이었다. 이건 못 쓴다 — 2차 차분은
아무리 작은 잡음에도 부호가 매 스텝 뒤집혀 **50% 근처로 포화**된다. 매끄러운
사인파로 직접 구동한 조(`DRIVE=spc`)에서도 49.2% 가 나왔다.

크기를 재는 두 지표로 갈아탔다:

    jerk_ratio   실측 |2차차분| 중앙값 / 지령 사인파의 이론 |2차차분|
                 1 이면 지령한 만큼만 움직인 것, 클수록 그 배수만큼 잡음이 얹힌 것
    |가속도| 잔차  2차차분 / dt^2 [m/s^2]. 주파수·dt 와 무관해 조 사이 비교가 된다

속도 부호 반전도 못 쓴다. 봉투가 큰 순드리프트로 이동 중이면 부호가 거의 안
뒤집혀(0.6%) "떨림 없음"으로 오독된다. 실제로 그렇게 한 번 오진했다.

### 25-2. 대조군이 원인을 갈랐다

파우더 유무만 바꾸고 나머지를 전부 고정한 대조다. |가속도| 잔차 중앙값 [m/s²]:

| 구동 방식 | 파우더 없음 | 파우더 524알(2g) |
|---|---|---|
| `spc` (강체 없이 SPC 목표를 사인파로) | **1.67** | 4.79 |
| `set` (강체를 set_pos 로만, 속도 갱신 없음) | **1.64** | 5.30 |
| `setvel` (set_pos + set_dofs_velocity) | **1.63** | 5.14 |

**파우더를 빼면 세 구동 방식이 소수점 둘째 자리까지 같고 `jerk_ratio` 가 1.0** —
봉투가 지령한 궤적만 따라간다. 파우더를 넣으면 3배로 뛴다.
...
논문 그림 후보는 `Crusher_Genesis/RESULT_논문용/` 에 있다(같은 시각 나란히 놓은
`compare_t*.png`).

### 25-3. `set_dofs_position` 순간이동 가설은 기각됐다

`run_arm` 이 매 스텝 `set_dofs_position` 으로 팔을 순간이동시키면서 속도는 갱신하지
않는다 — 접촉 솔버에 "움직이는 표면"이 아니라 "매 스텝 새 자리에서 밀어냄"으로
들어가니 이것이 원인이라고 봤다. **틀렸다.** 파우더 없는 조에서 `set`(1.64)과
`setvel`(1.63)이 같다. 순간이동이 문제였다면 여기서 갈렸어야 한다.

(스모크런 100알 2초에서는 `setvel` 이 41% 나아 보였는데 잡음이었다. 짧은 조로
방향을 정하면 안 된다.)

### 25-4. 실제로 듣는 지렛대

전 20조 중 파우더 있는 조들. |가속도| 잔차 중앙값 [m/s²]:

| 변인 | 범위 | 잔차 | 배수 |
|---|---|---|---|
| **`contact_d_hat`** | 5e-5 → 1e-4 → 5e-4 | 3.72 → 4.79 → **8.23** | **2.2배** |
| **낟알 수(같은 2g)** | 1768알(R0.5) → 524(R0.75) → 221(R1.0) | 3.68 → 4.79 → **5.38** | 1.5배 |
| 흔들기 진폭 | 20mm → 40mm | 2.89 → 4.79 | 1.7배 |
| 천 강성 `CLOTH_E` | 1e5 → 4e5 → 1.6e6 | 4.42 → 4.79 → 5.22 | 1.2배 |
| 굽힘 `CLOTH_BEND` | 100 → 400 → 1600 | 4.19 → 4.79 → 5.37 | 1.3배 |
| 구동 방식 | set / setvel / spc | 5.30 / 5.14 / 4.79 | **무관** |
| `dt` | 10 / 5 / 2.5 / 1.25 ms | 2.86 / 4.79 / 8.09 / 4.68 | **비단조** |

**`d_hat` 이 가장 강하다** — 낟알–천 접촉 배리어의 작용 범위가 곧 충격 크기다.
그리고 **같은 2g 이라도 곱을수록 조용하다**(개수가 많으면 개별 충격이 평균화된다).

`dt` 가 비단조인 것에 주의. 10ms 에서 가장 조용한데(2.86) 이건 덜 풀어서 뭉갠
것이지 좋아진 게 아니다. dt 는 이 문제의 지렛대가 아니다.

### 25-5. IPC 씬에서 조용히 죽는 노브들 — 목록

Genesis 가 legacy 솔버용으로 만든 노브가 IPC 커플러에서 **경고 없이 무시**된다.
값을 바꿔도 결과가 같으니 "효과가 없다"는 물리적 결론으로 오독하기 쉽다.
실제로 `FEM_DAMPING` 을 그렇게 읽을 뻔했다.

| 노브 | 상태 | 확인 방법 |
|---|---|---|
| `set_vertex_constraints` | 무반응 (§22, SPC 로 우회) | 소스에 참조 없음 |
| `SimOptions.substeps` | IPC 경로에서 안 읽힘 | `ipc_coupler/utils.py` 가 dt 만 넘김 |
| `FEMOptions.damping` | **커플러에 참조 자체가 없음** | `grep damping ipc_coupler/` → 0건 |

uipc 쪽 천은 `StrainLimitingBaraffWitkinShell`(moduli, mass_density, thickness) +
`DiscreteShellBending`(bending_stiffness) 이고 **감쇠 파라미터가 없다.** 천을
"덜 출렁이게" 하려면 감쇠가 아니라 E/굽힘/밀도로 가야 한다.

### 25-6. 남은 것

1. **흔들림 크기를 실기와 대조한 적이 없다.** "파우더가 봉투 거동을 지배한다"
   까지가 지금 말할 수 있는 것이고, 그 크기가 정량적으로 맞는지는 별개다.
2. `d_hat` 은 **전역**이라 정제·봉투 접촉에도 같이 영향을 준다. 낟알만 따로
   낮출 수 없다(libuipc config 의 `contact.d_hat` 하나뿐).
3. 격리 씬의 `set` 계열은 홀더가 봉투에 **접촉하지 않고** SPC 로만 매달린다
   (맞닿으면 초기 겹침으로 world 가 무효화된다). 그래서 **강체 접촉 경로**는
   이 스윕이 검증하지 못했다.
4. 왜 파우더가 이렇게 큰 반응을 만드는지는 미규명. 다음은 로봇팔과의
   상호작용을 다른 엔티티 없이 격리해서 본다.

드라이버 `probe/drivers/bag_shake_sweep.sh`, 로그 `probe/_shake_sweep/`,
데이터 `probe/RESULT_shake/*.npz`.

---

## 26. 이송 중 덜컥거림은 관성이 아니다 — 속도와 무관한 바닥이 있다 (2026-09-10)

§25 는 봉투를 **SPC**(소프트 위치구속)로 잡고 흔들었다. 실제 공정은 **마찰 파지**라
경계조건이 다르다. `probe/probe_arm_bag.py` 는 **로봇 + 봉투 + 파우더만** 남기고
(크러셔·회수장치·흡착장치·플레이트 전부 제외) 실제 파지를 재현한다.
상수와 로봇 MJCF 는 `import full_workflow as FW` 로 그대로 쓴다 — 복사해 두면
조용히 어긋난다.

### 26-1. 핑거와 봉투를 **둘 다** 잰다

`|가속도| 잔차 = |2차차분| / dt^2 [m/s^2]` 를 핑거 링크와 봉투 COM 양쪽에서 찍는다.
그래야 갈린다 — 핑거가 매끈한데 봉투만 덜컥이면 파지 경계, 핑거부터 덜컥이면
구동/솔버다.
...
| 운동 | 핑거 \|a\| | 봉투 \|a\| | 비 |
|---|---|---|---|
| 등속 직선 하강 150mm @ 20mm/s | 0.136 | **0.117** | 0.86 |
| lift (Q_GRASP→Q_LIFT, 200스텝) | 0.325 | 0.444 | 1.37 |
| **above** (Q_LIFT→슬롯 위, 400스텝) | 0.390 | **1.768** | **4.53** |

느린 등속 하강에서는 봉투가 핑거보다 **오히려 매끄럽다**. 덜컥거림은 `above`
구간에 국한된다. 그리고 **다른 엔티티가 하나도 없는 씬에서 재현된다** — 크러셔·
회수장치·흡착장치는 용의선상에서 빠진다.

`above` 는 `slot_geometry()` + `BAG_DX/DY_FROM_FINGER` + `VERTICAL_QUAT` 으로 IK 를
풀어 full_workflow 와 **같은 목표 자세**를 만들고 `ease()` 까지 동일하게 쓴다.
크러셔 엔티티가 없어도 되는 건 `slot_geometry()` 가 STL 과 좌표만 쓰기 때문이다.

### 26-3. 속도 사다리 — 관성이면 v^2 이어야 한다

경로를 그대로 두고 `N_ABOVE` 만 바꾼다(= 경로 동일, 속도만 변인). 관성력은
v^2 에 비례하므로 속도를 절반으로 하면 잔차가 4배 줄어야 한다.

| N_ABOVE | 속도 | 핑거 \|a\| | 봉투 \|a\| | v^2 예측(봉투) | 실측/예측 |
|---|---|---|---|---|---|
| 200 | 2.0배 | 1.394 | 1.739 | 6.340 | 0.3 |
| **400**(현행) | 1.0 | 0.369 | 1.585 | 1.585 | 1.0 |
| 800 | 0.5 | 0.241 | 1.002 | 0.396 | 2.5 |
| 1600 | 0.25 | 0.210 | 0.980 | 0.099 | **9.9** |
| 3200 | 0.125 | 0.153 | 0.615 | 0.025 | **24.8** |

**속도를 16배 늦췄는데 잔차는 2.8배밖에 안 줄었다.** 관성이면 256배 줄어야 한다.
멱법칙으로 맞추면 **봉투 |a| ∝ v^0.37** 이다(v^2 이면 지수 2.00).

`|a| = A·v^2 + C` 로 분해하면:

| | 관성항 A | **속도 무관 바닥 C** |
|---|---|---|
| 핑거 | 0.307 | **0.146** m/s^2 |
| 봉투 | 0.224 | **0.945** m/s^2 |

**현행 N_ABOVE=400 에서 측정된 1.585 중 60% 가 이 바닥이다.** 즉 이송을 늦춰도
덜컥거림이 안 없어진다 — 8배 늦춰도 0.615 에서 걸린다.

강체를 운동학적으로 구동하는 **핑거에도 0.146 의 바닥이 있다**. 물리적으로는
남을 이유가 없는 값이다.

### 26-4. 다음 후보 — Newton 수렴 허용치

바닥의 크기가 이걸 가리킨다. `newton_tolerance`(libuipc `newton.velocity_tol`)
기본값 **0.05 m/s** 는 dt=5ms 에서 스텝당 0.25mm 의 잔여 이동을 "수렴"으로
받아들인다는 뜻이다. 그게 매 스텝 무작위로 나타났다 사라지면 가속도로는
`0.05/0.005 = 10 m/s^2` 규모이고, 관측된 바닥 0.945 는 그 10% 다 — 자릿수가 맞는다.

§25-4 에서 `newton_tolerance=1e-3` 이 더미 높이를 28.7 -> 12.3mm 로 개선한 적이
있으나 **이 지표로는 아직 안 쟀다.**

### 26-5. 무효로 판정한 대조군 — 빈 봉투

"파우더 없음"을 대조군으로 쓰려 했으나 **빈 봉투는 애초에 파지되지 않는다.**
파지 유지 단계에서 이미 봉투가 -879mm 로 떨어지고(핑거는 +436mm 제자리),
지면이 없어 계속 떨어져 -280m 까지 간다. 발산이 아니라 자유낙하다.

순서 문제가 아니다 — full_workflow 도 형상 고정 해제 후 닫는 순서이고 파지 자세
(`q_grasp = Q_GRASP`)도 같다. 차이는 **내용물**이다. full_workflow 는 봉투에 항상
정제나 파우더가 들어 있어서 **속이 빈 봉투로 파지하는 경우가 실행된 적이 없다.**

따라서 "유동성이 원인인가"를 물으려면 질량은 같고 유동성만 다른 조가 필요하다:
낟알 마찰(0.0/0.6/1.5) 이나 장입량(0.5/1/2g) 이 유효한 설계다.

드라이버 `probe/drivers/above_speed_sweep.sh`, `arm_bag_sweep.sh`,
로그 `probe/_above_speed/`, 데이터 `probe/RESULT_armbag/*.npz`.

---

## 27. 분쇄 공정만 떼서 돌린다 — 충돌판을 빼면 때릴 것이 없다 (2026-09-10)

사용자 지시: "낟알이 들어가 있는 샘플백을 Crusher 로 때리는 공정만 따로 떼서
실행하고, 낟알에 가해지는 반력이나 sim 상에서 얻을 수 있는 데이터를 모두 수집."

### 27-1. `STAGE=crush` 만으로는 성립하지 않았다

§a300a0e 의 스테이지 분리는 `clamp` 이후에서 시작하면 `CLAMP_ONLY` 를 켠다.
그런데 `CLAMP_ONLY` 는 `patch_crusher_mjcf` 에서 **충돌판 `L9_PLATE_v3_1` 을
`contype/conaffinity=0` 으로 내린다** — 크랭크 q=0(빌드 자세)에서 판이 슬롯을
차지하고 있어, 거기 스폰한 봉투와 겹쳐 libuipc 가 world 를 무효화하기 때문이다.

**그 판이 곧 때리는 면이다.** 즉 종전 코드에서 `STAGE=clamp:crush` 는 크랭크만
헛돌고 봉투에는 아무것도 닿지 않는다. 원래 주석도 "Phase 11 crush 를 켜려면
CLAMP_ONLY 를 끄고 풀 시퀀스로 가야 한다"고 적고 있었다 — 즉 **분쇄만 떼는 경로는
존재한 적이 없었다.**

겹침 실측(`slot_geometry()` + 메시 AABB, 씬 없이 계산):

| 대상 | world AABB [mm] |
|---|---|
| 충돌판 `L9_PLATE_v3_1` (크랭크 q=0) | x[186.5, 236.5]  y[−8.52, +1.48]  z[25, 75] |
| 슬롯 스폰 봉투 | x[180.5, 244.5]  y[−8.27, −2.27]  z[51, 141] |
| 교차 | 50 × 6 × 24 mm |

y 로 6mm — 봉투 두께 전량이 판 안에 들어간다. 슬롯 간격이 12.5mm 뿐이라 y 로
비켜 놓을 자리도 없다.

### 27-2. 해법 — 형상을 빼지 말고 봉투를 띄운다

`CRUSH_LIVE = CRUSH_SECONDS > 0 and stage_on("crush")` 를 세우고:

1. **충돌판을 접촉에 남긴다** (`patch_crusher_mjcf` 의 분기 조건에 추가).
2. **봉투를 슬롯 위 `STAGE_SPAWN_DZ`(기본 100mm) 로 띄워 스폰**한다 — 판 z 상단
   75mm 위로 완전히 빠진다.
3. **prep 램프를 실제로 돈다.** `CLAMP_ONLY` 는 크랭크를 `set_dofs_position` 으로
   순간이동시키는데, 그러면 커넥팅로드/슬라이더 DOF 가 0 에 남아 weld 폐루프가
   모순된 자세에서 출발한다. 판을 접촉에서 빼 놨을 때는 드러나지 않던 문제다.
   200스텝 램프로 돌리면 기구가 제대로 접히고 슬라이더가 −40.04mm(완전 후퇴)에
   선다 = 판이 y[+31.5, +41.5] 로 슬롯을 비운다.
4. **SPC 로 내린다.** `rclock` 스테이지가 쓰는 것과 같은 처방(50mm/s, ease).
   하드 핀은 자기 정점만 옮겨 메시를 찢으므로 쓰면 안 된다(§22).

실측: 봉투 바닥이 목표 `wall_center_z=0.0510` 에 **오차 0.0mm** 로 안착한다.
크랭크는 분쇄 구간에서 8.00 RPM 에 정확히 도달한다(§21 의 `set`/`control` 혼용
문제는 이 경로에 없다 — 분쇄 중 벽은 `control_dofs_position` 이다).

스트로크 위상: 크랭크 −180deg 에서 슬라이더 −40.04mm(후퇴), 0deg 에서 0mm(전진).
8 RPM 이면 **첫 타격이 t≈3.75s, 이후 7.5s 마다** 한 번이다.

실행:

```bash
STAGE=clamp:crush GRAIN=1 GRAIN_WHEN=start SKIP_TABLET=1 RECOVER=0 \
CRUSH_SECONDS=60 RUN_TAG=crush_grain python -u full_workflow.py
```

`clamp` 를 같이 도는 이유: 봉투를 슬롯에 붙잡아 두는 것이 Left_Wall 압착이다.
`STAGE=crush` 만 주면 벽이 +6mm 개방 상태로 남아 첫 타격에 봉투가 밀려난다.
`SKIP_TABLET=1` 은 "낟알이 들어가 있는 샘플백"이라는 조건 그대로다 — 정제는
이 공정의 대상이 아니고, 빼지 않으면 파지 위치에서 자유낙하만 한다.

### 27-3. 수집 채널 — `crush_<tag>.npz`

| 채널 | 모양 | 무엇 |
|---|---|---|
| `dof_q/v/cf/f` | (n, 8) | 크러셔 전 관절 위치·속도·**액추에이터 출력**·외력버퍼 |
| `link_F/link_T` | (n, L, 3) | IPC→강체 커플링 반력/반모멘트 (링크별) |
| `g_fmean/fp95/fmax/fsum` | (n,) / (n,3) | 알당 접촉 반력 통계, 무리 합력 |
| `g_vmean/vmax/ke/ncon` | (n,) | 낟알 속력·운동에너지·접촉 중인 알 수 |
| `g_com/lo/hi`, `b_com/lo/hi` | (n, 3) | 낟알 더미 / 봉투 AABB |
| `fld_pos/vel/f` | (m, N, 3) | 낟알 **전량** 좌표·속도·반력 |
| `fld_nn/fld_dbag` | (m, N) | 알별 최근접 이웃 거리 / 봉투 표면 거리 |
| `bagv_pos` | (b, V, 3) | 봉투 정점 좌표 |

**반력을 어디서 읽는가 — 채널 셋이 서로 다른 것을 잰다.**

- `get_dofs_control_force()` = kp·(cmd−pos)+kv·(vel−cmd_vel) 을 `force_range` 로
  클램프한 값 = **액추에이터가 실제로 낸 출력**. 봉투/낟알이 되미는 만큼 커진다.
- `get_dofs_force()` = `dofs.force` 버퍼. probe 씬(강체 단독)에서는 잔여값 ~0
  이었지만(§8de08ac), full_workflow 에서는 IPC 커플러가
  `apply_links_external_force` 로 접촉 반력을 그 버퍼에 실어 보내므로 다른 값이
  나온다(실측: 벽 control_force −100N 포화 구간에서 dofs_force −183N).
- `coupler._coupling_data.out_forces/out_torques` = k_trans·m·(x_ipc − x_aim).
  **접촉력이 아니라 페널티 항이다 — 크기를 물리량으로 읽으면 안 된다.**
  `_constraint_strength_translation_scaled = constraint_strength_translation/dt²`
  = 100/(5e-3)² = **4×10⁶ N/(kg·m)** 이라, 0.3kg 링크가 0.1mm 만 뒤처져도 120N 이
  나온다. 실측에서 충돌판 링크 `L8_Link3_Shaft_1` 이 최대 **23.9 kN** 을 찍는데,
  이건 크랭크가 기구학적으로 끌고 가는 링크라 IPC 쪽 자세가 매 스텝 뒤처져서
  생기는 값이지 낟알이 판을 그만큼 되민 것이 아니다.
  **쓸모 있는 것은 크기가 아니라 시점**이다 — 타격 순간에 정확히 스파이크가
  서므로 타격 검출기로는 훌륭하다. 하중의 크기는 액추에이터 채널(크랭크 토크,
  벽 반력)에서 읽어야 한다.

**낟알 반력은 운동량에서 만든다.** uipc 는 입자별 접촉힘을 밖으로 내주지 않는다.
`v^k = (x^k − x^{k−1})/dt`, `f^k = m(v^k − v^{k−1})/dt − mg` 로 중력을 뺀 나머지가
이웃 낟알/봉투/기구가 그 알을 민 합력이다. backward-Euler 적분과 정합한다.

> **builtin `velocity` 속성은 쓰면 안 된다.** 읽기는 되는데 값이 항상 0 이다
> (2026-09-10 실측: `fld_vel` 전량 0 인데 같은 구간 좌표는 5스텝에 최대 1.27mm
> 움직였다). 커플러가 uipc 에서 매 스텝 되받는 것은 좌표뿐이고
> (`_retrieve_grain_states`), 속도는 프런트엔드 지오메트리에 되쓰이지 않는다.
> 이걸 모르고 쓰면 반력이 정확히 자중과 같아져 "낟알이 아무 힘도 안 받는다"는
> 헛것을 본다.

첫 표본은 `v^{-1}=0` 가정 때문에 가짜 임펄스가 섞이므로 요약에서 버린다.

플롯/요약: `plot_crush_data.py [경로.npz]` — 채널 인벤토리를 찍고 하중 경로
(크랭크 토크 → 벽 반력 → 커플링 반력 → 알당 반력 → 압밀) 를 한 장으로 그린다.

### 27-4. 실측 — 8 RPM x 60s, 낟알 500알(R=0.5mm, 0.393g) 든 봉투

`RESULT_crush_grain2/grip-28mm/crush_*.npz` (60.1MB, 12,000점). 13,662스텝 27분.
clamp 종료 −14.59mm / 본체면 잔여 2.91mm 로 풀 시퀀스(`_grain_full3`: −14.54mm /
2.96mm)와 일치 — **격리 스테이지가 풀 파이프라인의 압착 조건을 재현한다.**

슬라이더 스트로크 −42.94 ~ −0.65mm (설계 40mm + 컴플라이언스).

사이클별(크랭크 1회전 = 7.5s):

| 주기 | t [s] | τ 최대 | τ 최소(후퇴) | 벽 밀림 | 더미두께 최소→회복 | 알당 \|f\| 최대 |
|---|---|---|---|---|---|---|
| 1 | 0.0~8.4 | **+12.50** | −10.42 | +3.390mm | 0.64 → 6.31mm | 0.1576 mN |
| 2 | 8.4~15.9 | **+12.50** | −7.11 | +0.559mm | 0.68 → 3.46mm | 0.0422 mN |
| 3 | 16.0~23.5 | **+12.50** | −6.50 | +0.482mm | 0.77 → 2.97mm | 0.0491 mN |
| 4 | 23.5~31.0 | **+12.50** | −6.04 | +0.442mm | 0.84 → 2.98mm | 0.0536 mN |
| 5 | 31.0~38.5 | **+12.50** | −5.69 | +0.415mm | 0.81 → 2.96mm | 0.0454 mN |
| 6 | 38.5~46.0 | +12.36 | −5.50 | +0.375mm | 0.87 → 2.98mm | 0.0479 mN |
| 7 | 46.0~53.5 | +11.75 | −5.09 | +0.376mm | 0.86 → 2.96mm | 0.0367 mN |
| 8 | 53.5~60.0 | +11.52 | −4.93 | +0.406mm | 1.00 → 3.05mm | 0.0608 mN |

**8주기 전부 타격한다.** 1~5주기는 토크 상한 12.5 N·m 에 포화하고 6~8주기는
11.5~12.4 로 조금씩 내려간다. 후퇴 토크도 −10.4 → −4.9 로 단조 감소하고, 더미
압축 하한이 0.64 → 1.00mm 로 올라간다 — **타격을 거듭할수록 낟알층이 덜 눌린다.**
1주기 값들(벽 밀림 3.39mm, 알당 0.158mN)은 분쇄 시작 시 SPC 해제 과도라 빼고 읽어야 한다.

전 구간: 벽 반력 평균 −14.65N / 중앙 −14.40N (상한 100N 포화는 0.02% 스텝뿐),
크랭크 토크 평균 +1.320 N·m, 크랭크가 60초에 넣은 일 **+56.2 J**.

### 27-5. 낟알 반력을 운동량으로 읽을 때의 한계 — 준정적에서는 알짜힘이 0 이다

알당 반력 **평균 0.0077 mN = 자중과 정확히 같다.** 타격 중 최대도 0.037~0.061 mN
(자중의 5~8배)에 그친다. 이건 계측 실패가 아니라 정의의 문제다 — 운동량 역산은
**알짜힘**이고, 8 RPM 준정적 압착에서 알은 이웃 사이에 끼어 눌려도 가속하지 않아
알짜힘이 0 에 수렴한다. 접촉 하중(알이 서로 미는 힘)은 이 채널로 안 잡힌다.

낟알이 실제로 움직이는 것은 잡힌다: 최대 속도 1,023.6 mm/s, 무리 운동에너지 최대
48.4 uJ, 무리 합력 평균 3.863 mN(전체 자중 3.852 mN)·최대 38.3 mN.

### 27-6. 더 큰 문제 — 낟알층이 IPC 배리어에 굳어 있다

알별 최근접 이웃 거리(`fld_nn`) 분포:

| | 중앙 | 5% | 95% | 배리어 ±1% 이내 | 배리어보다 가까움 |
|---|---|---|---|---|---|
| 분쇄 시작 | 1.0967 | 1.0841 | 1.0992 | **92.2%** | 7.6% |
| 중간 | 1.0991 | 1.0940 | 1.0998 | **96.2%** | 3.2% |
| 종료 | 1.0986 | 1.0904 | 1.0998 | **94.4%** | 4.4% |

배리어 `2R + d_hat` = 1.100mm. **낟알의 92~96% 가 정확히 그 거리에 붙어 있다** —
서로 닿은 적이 없고, 타격을 60초 받아도 그 비율이 오히려 올라간다. §GRAIN 주석의
N884(d_hat/R=1.0) 사례와 같은 비계(scaffold) 상태가 **d_hat/R=0.2 에서도** 난다.

결론: 이 조건에서 시뮬은 **분말의 입자간 접촉 응력을 풀고 있지 않다.** 낟알층은
배리어가 만든 준강체 비계이고, 크러셔가 그것을 통째로 밀었다 놓는 것이 지금 잡히는
현상이다. 27-4 의 토크·벽 반력은 그 비계를 미는 하중이라 실기 대응이 되지만,
"낟알 하나가 몇 N 을 받아 깨지는가"는 이 설정으로는 답이 안 나온다.

바꿀 축(미검증): `d_hat` 을 낮춰 배리어 껍질을 얇게 하거나(다만 정제·봉투까지 같이
바뀐다), 낟알 반지름을 키워 d_hat/R 비를 더 내리거나, `newton_tolerance` 를 조인다
(§26-4 가 지목한 축이고 §25-4 에서 더미 높이를 28.7→12.3mm 로 바꾼 전력이 있다).

### 27-7. SPC 해제는 **개수**가 아니라 **힘**을 램프해야 한다 (2026-09-11)

격리 스테이지는 봉투를 SPC 로 세워 두고 분쇄 시작에 푼다. 그걸 한 스텝에 풀면
구속이 대신 받던 하중(자중 + 벽 압착 반력)이 통째로 임펄스가 된다.

**1차 시도(틀림) — 구속 정점 수를 246 -> 0 으로 줄였다.** 더 나빠졌다:

| | 해제창 알당 \|f\|max | 해제창 v_max | 1주기 벽 밀림 |
|---|---|---|---|
| 즉시 해제 | 0.1576 mN | 1,023.6 mm/s | 3.390 mm |
| 개수 램프 | **0.2678 mN** | **1,662.4 mm/s** | **6.269 mm** |
| 힘 램프 | **0.0422 mN** | **257.1 mm/s** | **1.592 mm** |

피크가 해제 **시작이 아니라 끝**(t=1.825s)으로 옮겨갔다. 정점을 하나씩 놓으면
하중이 남은 정점으로 **몰린다** — 마지막 몇 개가 봉투 전체를 혼자 버티다 터진다.
**개수를 줄이는 것은 힘을 줄이는 것이 아니다.**

**2차(맞음) — 전 정점을 계속 구속한 채 `aim` 을 각 정점의 현재 위치로 수렴시킨다.**
SPC 힘이 `k*(aim - x)` 이므로 `aim -> x` 면 힘이 0 으로 간다. 전 정점이 동시에
같은 비율로 힘을 놓아 몰림이 없다. (`strength_rate` 는 `apply_to` 로 빌드 때
박히므로 런타임에 못 바꾼다 — `aim` 을 움직이는 것이 같은 효과의 우회다.)

해제 과도가 **정상 타격 반응(0.0593 mN)보다 작아졌다**(0.0422). 더 중요한 것은
과도가 **이후 60초 전체를 바꾼다**는 점이다:

| | 주기별 τmax [N·m] | 압축 하한 [mm] | 더미 높이 [mm] |
|---|---|---|---|
| 즉시 해제 | 12.50→11.52 (감소) | 0.64→1.00 (표류) | 7.0 → 18.9 |
| 개수 램프 | 12.50→9.71 (급감) | 1.30→2.14 | 4.6 → 23.2 |
| **힘 램프** | **12.50 전 주기 일정** | **0.81~0.86 일정** | 4.1 → **7.5** |

해제가 셀수록 낟알이 위로 튀어 **타격 구역을 벗어나** 이후 주기의 하중이 줄어든다.
힘 램프는 낟알을 제자리에 두므로 매 주기가 같은 조건으로 반복된다 — 타격 이력을
비교하려면 이게 전제다. 봉투 형상도 유지된다(폭 15.8 vs 25.2/29.7mm).
분쇄 구간 계산도 92.2 -> 71.4 ms/스텝으로 빨라진다.

노브: `BAG_SPC_RELEASE_STEPS`(기본 400 = 2.0s), 0 이면 즉시 해제.

**배리어 잠금은 이걸로 안 풀린다**(예측대로): 배리어 ±1% 이내 비율이 84.0% ->
97.6%. 과도를 부드럽게 하는 것과 `2R+d_hat` 이라는 기하 상수는 무관하다.

### 27-8. 영상이 분쇄 경계에서 끊겨 보이는 이유 — 렌더 간격 (2026-09-11)

물리가 아니라 렌더다. 분쇄 밖은 매 스텝, 분쇄는 `CRUSH_RENDER_EVERY=10` 으로
10스텝마다 한 프레임이라 **프레임 하나가 담는 시뮬 시간이 경계에서 10배 뛴다**.
앞은 슬로모션, 분쇄부터 빨리감기가 된다. `RENDER_EVERY`(기본 1)를
`CRUSH_RENDER_EVERY` 와 맞추면 재생속도가 일정해진다.

### 27-9. 낟알을 키우면 배리어가 풀린다 — 방향이 뒤집혔다 (2026-09-11)

§27-6 의 배리어 잠금을 푸는 축으로 **`d_hat` 대신 R 을 키웠다**(`d_hat=1e-4` 은
정제·봉투까지 같이 바꾸므로 못 건드린다). 봉투 내부 최대 여유가 3.00mm 라
장입에 상한이 있다 — 요구 여유 = `CLOTH_THICK + R + d_hat + 0.2mm`:

| R | d_hat/R | 여분간격(2R 대비) | 최대 장입 | 총 질량 |
|---|---|---|---|---|
| 0.5mm | 0.20 | 10.0% | 2000+ 알 | (500알 = 0.393g) |
| **1.0mm** | **0.10** | **5.0%** | 382알 | **2.40g** |
| 1.5mm | 0.067 | 3.3% | 140알 | 2.97g |

R=1.0mm / 332알(2.086g)로 60초 타격:

| | R=0.5mm 500알 0.393g | R=1.0mm 332알 2.086g |
|---|---|---|
| 배리어 ±1% 이내 | 84.0% -> **97.6%** (굳어감) | 98.8% -> **65.7%** (풀림) |
| 배리어 ±0.2% 이내 | 39.8% -> **93.0%** | 86.4% -> **34.9%** |
| 크랭크 토크 평균 | 2.862 N·m | **3.645 N·m** |
| 토크 포화 스텝 | 20.5% | **27.4%** |

**방향이 뒤집혔다.** R=0.5 에서는 때릴수록 배리어에 더 굳는데, R=1.0 에서는
때릴수록 **풀려난다.** 크랭크가 해야 하는 일도 27% 늘어 낟알층이 실제로
저항하기 시작한다.

**교란 요인**: R 을 키우면 알 질량(8배)과 총 질량(5.3배)이 같이 움직인다. 배리어
하나만 분리하려면 R=0.5mm 로 2,800알을 넣어 질량을 맞춘 대조군이 필요하다(미실행).
다만 2.086g 는 프로젝트 목표 장입량 2g 에 맞는 값이라, 실공정 조건으로는 오히려
이쪽이 맞다 — 종전 0.393g 가 실제보다 훨씬 적었다.

### 27-10. 배치 스틸 카메라 (2026-09-11, 사용자 지시)

종전 `LAYOUT_ONLY` 스틸은 `(1.30,-1.30,0.95)` / `cam_over` 의 fov 48 로, 대상까지
1.96m 에 세로 1.75m 를 담아 **장치가 화면의 1/3** 밖에 안 됐다. 고친 것:

- **레이아웃 전용 카메라 신설** — fov 는 `add_camera` 시점에 고정이라
  (`set_pose` 로 못 바꾼다) 화각을 바꾸려면 카메라를 따로 만들어야 한다.
  기본 fov 34, 해상도 1920x1440.
- **프레이밍을 계산으로** — 장치 5개 좌표에서 중심을 잡고 `dist = R/tan(fov/2)`
  로 거리를 역산한다. 하드코딩 제거.
- **로봇을 `Q_GRASP` 로 세우고 촬영** — 빌드 직후는 MJCF 기본자세(관절 0)라 팔이
  수직으로 1.2m 뻗어, 장치를 크게 담으면 팔이 잘리고 팔을 담으면 장치가 작아진다.
  크랭크 -180deg(후퇴), 벽 +6mm(개방)로 운전 시작 상태도 같이 만든다.
- **뷰를 환경변수로** — `LAYOUT_VIEWS="이름:방위각:고도각,..."`.

**정면은 방위각 -90deg 다.** 4방향(-90/0/90/180)을 고도 0 으로 찍어 비교한 결과
여기서만 크러셔 브라켓 면이 정면을 보고 로봇이 장치를 가리지 않는다. 고도 0 =
수평 시선이라 알루미늄 판이 화면에서 정확히 수평선이 된다(틀어짐 없음).
고도 90(진짜 평면도)은 시선과 기본 up 벡터가 평행해 자세가 정의되지 않으므로
그 뷰만 up 을 +Y 로 준다.

```bash
LAYOUT_ONLY=1 LAYOUT_VIEWS="정면도:-90:0,평면도:-90:90,정면상단45:-90:45" LAYOUT_R=0.52 LAYOUT_CZ=0.16 python -u full_workflow.py
```

**남은 것**: 런 영상의 `cam_over` 는 여전히 `(0.9,-1.7,1.3)`/fov 48 로 멀다.
같은 계산을 거기에도 넣으면 이후 모든 런의 영상 프레이밍이 바뀌므로 보류한다.

### 27-11. 입자별 **접촉력**을 uipc 에서 꺼낼 수 있다 — §27-5 의 한계가 풀린다 (2026-09-11)

§27-5 는 낟알 반력을 좌표 2계 차분(= **알짜힘**)으로만 냈고, 그래서 준정적
압착에서 값이 자중으로 수렴해 **접촉 하중을 못 쟀다.** 양옆에서 10N 씩 눌리는
알은 알짜힘이 0 인데, 정제를 깨는 것은 바로 그 눌리는 하중(force chain)이다.

사용자 질문("입자별 접촉력을 알 수 없는데 어떻게 입자마다 거동이 다른가, 단순히
uipc 내부 값이라 확인을 못 하는 건가")에서 출발해 확인했다. **답: 배선 문제였고,
뚫린다.**

#### 경로

```python
csf = world.features().find(uipc.core.ContactSystemFeature)   # 'core/contact_system'
g = uipc.geometry.Geometry()
csf.contact_gradient(prim_type, g)      # g.instances() 에 채워진다
    grad : (n,3,1)   정점별 dE_contact/dx
    i    : (n,)      전역 정점 인덱스  <- 알마다 귀속시킬 수 있다
접촉력 = -grad / dt^2
```

`world.features()` 가 내주는 것은 셋이다 — `core/contact_system`,
`core/finite_element_state_accessor`, `core/distance_diagnoser`.

`prim_type` 은 10종: `PT/PP/EE/PE/PH` x `N`(법선)/`F`(마찰).
낟알은 점군이라 **낟알-낟알이 `PP`, 낟알-바닥평면이 `PH`** 다. 해당 접촉이 없는
타입은 size 0 으로 돌아온다(에러 아님).

#### dt^2 스케일이 핵심이다

IPC 는 증분 포텐셜 `E = 0.5|x-x~|^2_M + dt^2*(E_elastic + E_contact)` 를
최소화하므로 gradient 에 **dt^2 이 이미 곱해져 있다.** 모르고 그대로 읽으면 힘이
4만분의 1 로 나온다.

정적평형 검산(낟알 120알 R=1.0mm, 바닥 평면, 4,000스텝 정착):

| | |
|---|---|
| 무게 / 원시 z합 | **40000.0** |
| `1/dt^2` (dt=5e-3) | **40000.0** |
| 보정 후 z합 / 총무게 | **1.0000** |
| 정점별 \|f\| 중앙 = 90% = 최대 | **0.0616 mN = 1알 자중** |

정착 후 120알이 전부 한 층으로 깔려 각자 제 무게만 받는 상태라, 검산이
다섯 자리까지 맞는다. `PH+N` 항목이 정확히 120개(모든 알이 바닥과 접촉)다.

**함정**: 600스텝만 정착시켰을 때는 이 비가 **6640** 으로 나왔다. 더미가 안
멈추면 정적평형이 성립하지 않아 검산이 무의미하다. 반드시 속도가 충분히 0 에
간 뒤에 재야 한다(프로브가 정착 중 속도를 같이 찍는다).

#### 무엇이 달라지나

- **force chain 을 직접 읽는다.** 알짜힘이 아니라 눌리는 하중이라, 준정적
  압착에서도 0 으로 수렴하지 않는다.
- **법선(`N`)과 마찰(`F`)이 분리돼 있다.**
- **알마다 귀속된다** — `i` 가 전역 정점 인덱스라 낟알 인덱스로 매핑된다.

참고로 **거동 자체는 원래도 관측되고 있었다.** R=1.0mm 런 3번째 타격
(t=21.5~22.3s, 332알) 실측: 알별 변위 중앙 1.578mm(5% 0.793 / 95% 2.723),
무리 평행이동 1.133mm, **평행이동을 뺀 개별 성분이 중앙 1.200mm 로 무리 이동보다
크다.** 이웃 5개까지의 거리도 중앙 0.032mm / 95% 0.389mm 변한다 — 강체 덩어리가
아니라 재배열하는 더미다. 못 보던 것은 거동이 아니라 **힘 하나**였다.

프로브: `probe/probe_contact_force.py` (근거·함정·검산 포함).

**배선 완료(2026-09-11)** — 그리고 그 값이 힘이 아니었다는 것이 §27-12 에서 드러난다.
~~미배선: `full_workflow.py` 의 `_record()` 에는 아직 안 넣었다. 넣으면
R=1.0mm 런에서 "낟알 하나가 몇 N 받는가"가 바로 나오지만, 값을 얻으려면 60초
런을 다시 돌려야 한다(~21분).~~

### 27-12. 접촉력이 힘이 아니라 **Newton 잔차**였다 — 수렴 조건 규명 (2026-09-23)

사용자 질문 "지금 계산되는 접촉력이 얼마나 정확한가, 알별로"에서 출발했다.
답: **알별로는 못 쓴다.** 그리고 왜 그런지가 §27-11 의 검산 절차에 이미 적혀
있었는데 분쇄 런에서 그 조건이 성립하지 않는다는 걸 아무도 안 봤다.

#### 기준이 되는 식은 독립 측정이 아니라 **같은 식의 반대편**이다

자유입자는 IPC 증분 포텐셜의 최소점에서

    E(x) = 0.5|x - x~|^2_M + dt^2 * E_ct(x),   x~ = x^{k-1} + dt*v^{k-1} + dt^2*g
    ∇E = 0  ->  M(x - x~)/dt^2 + ∇E_ct = 0
    ->  f_ct = -∇E_ct = m[(v^k - v^{k-1})/dt - g]

오른쪽이 `_record()` 의 운동량 역산식 그대로다. 즉 두 채널(접촉 gradient /
좌표 2계차분)은 **한 방정식의 양변**이고, 수렴했으면 반드시 같다. 다르면 그
차이가 Newton 잔차다. "역산이 참값"이 아니라 "솔버가 자기 식을 안 풀었다"는
뜻으로 읽어야 한다.

#### 실측 — `RESULT_cfprofile0922`(R=0.5mm 500알, 8 RPM x 60s, 기본 tol)

| 알-스텝 120만 표본 | |
|---|---|
| \|f_cf\|/\|f_mom\| 중앙 | **22.4배** (p5 1.29, p95 8,714) |
| 참값의 2배 이내 / 10배 이내 | **8.6%** / 35.4% |
| 방향 일치 cos 중앙 | **0.365** |
| 무리 합력 z / 총자중 — 운동량 | **1.0000** |
| 무리 합력 z / 총자중 — 접촉 gradient | **1,896** |

원인은 §GRAIN 주석이 이미 지목해 둔 것이다: libuipc 기본
`velocity_tol = 0.05 m/s` 인데 분쇄 중 낟알 속력 중앙이 **1.33 mm/s** 다.
솔버가 낟알 운동을 잡음으로 보고 끊는다. `probe_contact_force.py` 가 그 채널을
검증한 조건은 4,000스텝 정착시킨 **정지** 더미(µm/s)였고, 600스텝 미정착에서는
비가 6,640 이었다고 그 스크립트가 직접 적어 뒀다 — 같은 함정이다.

#### 수렴 스윕 — `probe/drivers/newton_conv_sweep.sh` (7런, R=1.0mm 332알 15s)

| 런 | ms/스텝 | 검산 중앙 | \|오차\|<10% | 알별 비 | 2배 이내 | cos |
|---|---|---|---|---|---|---|
| R=0.5mm 500알, 기본 tol | 117 | 50.4 | 0.1% | 22.33 | 9.9% | +0.483 |
| R=1.0mm 332알, 기본 tol | 96 | 4.0 | 10.4% | 4.91 | 32.5% | +0.743 |
| `NEWTON_TOL=1e-3` | 224 | 1.0 | 71.9% | 1.02 | 81.7% | +0.996 |
| **`NEWTON_TOL=1e-4`** | **332** | **1.0** | **99.8%** | **1.00** | **99.9%** | **+1.000** |
| `1e-5` | 994 | 1.0 | 99.9% | 1.00 | 100.0% | +1.000 |
| `1e-5` + `NEWTON_MAX_IT=200` | 1,612 | 1.0 | 99.6% | 1.00 | 100.0% | +1.000 |
| `1e-5` + `EPS_VEL=1e-4` | 1,562 | 1.0 | 99.7% | 1.00 | 99.9% | +1.000 |

**`NEWTON_TOL=1e-4` 가 최적점이다.** 더 조여도 수렴율이 안 오르고 비용만 3~4.7배
붙는다. 반복 상한은 범인이 아니었다(200 으로 올려도 같은 값에 1.6배만 느림).

#### 결함이 둘이고 **독립**이다

알 크기는 수렴을 12배 개선했지만(검산 중앙 50.4 -> 4.0) **배리어는 못 풀었다** —
일곱 런 전부 최근접 간격 중앙이 배리어 `2R+d_hat` 의 ±0.5% 안이고, tol 을 100배
조여도 2.098~2.099mm 에서 한 자리도 안 움직인다. §27-9 가 60초 런에서 본 해제는
15초 구간에서는 안 일어난다.

    잔차 문제   -> NEWTON_TOL=1e-4 로 해결
    배리어 문제 -> 미해결. tol 로는 영원히 안 풀린다.

즉 지금 읽히는 알별 하중(443 mN, p95 2.8 N)은 **"수렴한 배리어력"**이다. 계산은
정확하지만 값을 정하는 것은 알의 물성이 아니라 `d_hat`·`contact_resistance` 다.
사용자 지적 그대로다 — "접촉력은 IPC 커플러 파라미터가 결정하는 구조".

남은 선택지: (a) `d_hat` 을 낮춘다(전역이라 정제·봉투 동반 변경), (b)
`contact_resistance` 를 실제 알갱이 접촉 강성에 맞춰 캘리브레이션한다(낟알
쌍별 지정 가능, `ipc_grain_coupler.add_grains`). PINN 학습 데이터로 절대값을
쓰려면 (b) 가 선결이다.

계측 노브: `NEWTON_TOL`, `NEWTON_MAX_IT`(신설), `EPS_VEL`. 런마다
`[RESULT] 수렴 검산`이 찍히고 50% 미만이면 경고가 붙는다.

### 27-13. 접촉을 **쌍 그대로** 남긴다 — 파생량을 없앴다 (2026-09-23)

사용자 지시: "논문에는 복잡한 수식을 쓸수록 검증절차 또한 복잡해진다."

§27-11 배선은 정점별 gradient 를 알 ID 로 **모았다**. 모으는 순간 "누가 누구를
밀었나"가 사라지고, 알당 값 하나를 쓰려면 벡터합/크기합/최댓값 중 무엇이냐를
매번 정당화해야 한다. (실제로 §27-11 은 "알짜힘이 아니라 눌리는 하중"을 잡겠다고
뚫은 채널인데 벡터합이라 §27-5 와 같은 상쇄를 되풀이하고 있었다.)

#### gradient 는 **이미 접촉별 행**이다 — 합쳐져 있지 않다

`probe/probe_contact_pairs.py` 실측(12알 한 줄 기둥, R=1.0mm):

    PP+N contact_gradient  instances=22  i(22,)int32  grad(22,3,1)float64
    PP+N contact_hessian   instances=33  i(33,) j(33,) hess(33,3,3)

22 = 11접촉 x 2정점, 33 = 11접촉 x 블록 3개(ii, ij, jj). 헤시안에는 `i`,`j` 가
있지만 **거기까지 갈 것도 없다** — gradient 를 arity 만큼 끊어 읽으면 접촉이
그대로 복원된다:

    PP 점-점 2행 · PE 점-모서리 3행 · PT 점-삼각형 4행 · EE 4행 · PH 점-평면 1행

검산 세 줄이 전부 통과한다:

| | |
|---|---|
| 뉴턴 3법칙 \|g_k + g_{k+1}\|/\|g_k\| | **< 1e-9** (11쌍 전부) |
| 힘 방향 — 두 알을 잇는 축과 \|cos\| | **1.000000** |
| 정적평형 — k번째 접촉 / 1알 자중 | **정확히 k배** (0.0616 … 0.678 mN) |

마지막 줄이 핵심이다. 12알 기둥에서 위에서 k번째 접촉은 그 위 k알의 무게를 받아야
하는데 정확히 그 값이 나온다. **검증 절차가 한 줄로 끝난다** — 수식 유도도, 상쇄
해명도 없다.

#### 저장 형태 (CSR)

스텝마다 접촉 수가 달라 2차원 배열이 안 된다.

| 채널 | 모양 | 무엇 |
|---|---|---|
| `pair_step` | (m,) | 표본 스텝 |
| `pair_off` | (m+1,) | 각 표본의 구간 |
| `pair_i` | (P,) | 낟알 **로컬 ID** = `get_grain_positions()` 행 번호 |
| `pair_j` | (P,) | 상대 **전역 정점 인덱스**. `grain_vert_lo<=j<grain_vert_hi` 면 낟알(로컬 = `j-lo`), 아니면 봉투/기구. PH 는 −1 |
| `pair_t` | (P,) | 프리미티브 타입 = `cf_type_names` 인덱스 |
| `pair_f` | (P,3) | 그 접촉이 알 `i` 에 가한 힘 [N] |

파생량이 없다. 알당 값이 필요하면 **읽는 쪽에서** 모으고, 모으는 방식은 그 분석이
책임진다. 노브 `CRUSH_PAIR_EVERY`(기본 = `CRUSH_FIELD_EVERY`), 0 이면 끈다.
arity 가정이 깨지는 타입은 조용히 넘기지 않고 경고 후 그 타입만 뺀다.
