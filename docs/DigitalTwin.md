...
조 간격이 16mm 아래로 좁혀지는 순간부터 튄다. 좌우가 0.3mm 안쪽으로 같이 움직이니
미믹 문제는 아니다. 명령이 전혀 안 바뀐 **스테이지까지 ±1.9mm 흔들린다.**

가설은 둘이다 — (a) 컵끼리 자가충돌, (b) 0 까지 닫는 구동 자체.
변인통제로 MJCF 사본에서 조 range 를 `-0.05 ~ -0.003` 으로 제한해 가른다
(`SUCK_JAW_MIN_MM`). **결과 미확정.**
