...
1. **회수장치 투입 자세 [1순위]** — yaw 90°(실링 정렬)와 낮은 tilt 가 현재
   트레이드오프다. `rcdown` 감속 또는 하강 전 정착 구간이 후보다.
   기준선은 `RESULT/grip-28mm/*_ad_plan.mp4` (tilt 2.0°, 낙하 0.0mm, 수평오차 27mm).
1-b. 플랜지 접촉 ON 복귀 — §21-9 의 두 가지(분쇄 중 벽 유지, `unclamp` 기점)를
   고쳐야 한다. 1순위 작업이 끝난 뒤.
