...
### 19-5. 남은 문제 — insert 에서 기운다 **[미해결]**

자세를 고친 뒤에도 삽입은 실패한다.

| | 자세 수정 전 | 자세 수정 후 |
|---|---|---|
| baseline width_y | 64.3mm | **6.4mm** (정상) |
| aboveset tilt | 2.2deg | 1.2deg |
| **insert tilt** | 22.8deg | **25.2deg** |
| 오차 | +25.0mm | +25.7mm |

파지·리프트·이송은 tilt 1.2deg 이하로 안정적이고, aboveset 시점에 봉투 X 범위
[0.180,0.244] 가 슬롯 X 범위 [0.180,0.245] 안에 정렬돼 있다. 그런데 하강 중 X 로
**9.4mm** 밀린다 — 폭 64mm 대 슬롯 65mm 라 여유가 편측 0.5mm 뿐이라 그만큼이면
바로 걸린다. 후보 둘:

1. **손목 트위스트 부호** — `WRIST6_DEG=+90` 으로 넣었으나 −90 일 수 있다. 파지는
   성립했지만 반대쪽 면을 물고 있으면 하강 중 모멘트가 생긴다. 환경변수로 분리해
   뒀으므로 한 런으로 갈린다.
2. **하강 중 팔 간섭** — 손목이 90도 돌며 팔 자세가 바뀌었다. §14 의
   `slot_ik_check.py` 충돌 확인을 실배치에서는 아직 안 돌렸다.
